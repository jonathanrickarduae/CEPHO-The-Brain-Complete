...
export function QADataClassificationBadge({
  ...
  if (compact) {
    return (
      <span
        className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs ${config.bgColor} ${config.color} ${config.borderColor} border`}
      >
...
export function QASummary({
  ...
  const verifiedCount = items.filter(
    i => i.classification === "verified_fact"
  ).length;
  ...
  return (
    <div className="p-4 bg-card/50 rounded-xl border border-white/10">
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {(Object.keys(classificationConfig) as DataClassification[]).map(
          classification => {
            const config = classificationConfig[classification];
  ...
  return (
    ...
    >
      ...
      {methodology && classification === "estimate" && (
        <div className="mt-1 text-xs text-amber-400/80">Est: {methodology}</div>
      )}
    </div>
  );
}
