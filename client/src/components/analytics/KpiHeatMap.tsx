...
export function KpiHeatMap({
  ...
  const [selectedDomain, setSelectedDomain] = useState<KpiDomain | "all">(
    "all"
  );
  ...
  const scoresByCategory = useMemo(() => {
    const map = new Map<
      number,
      { cos: number; sme: number; customer: number; average: number }
    >();

    KPI_CATEGORIES.forEach(cat => {
      const cosScore =
        categoryScores.find(
          s => s.categoryId === cat.id && s.assessorType === "chief_of_staff"
        )?.score ?? 0;
      const smeScoresForCat = expertScores.filter(s => s.categoryId === cat.id);
      const smeAvg =
        smeScoresForCat.length > 0
          ? smeScoresForCat.reduce((sum, s) => sum + s.score, 0) /
            smeScoresForCat.length
          : 0;
      const custScore =
        customerScores.find(s => s.categoryId === cat.id)?.score ?? 0;
  ...
  const outliers = useMemo(() => {
    ...
    expertScores.forEach(es => {
      const catScores = expertScores.filter(
        s => s.categoryId === es.categoryId
      );
  ...
  const overallStats = useMemo(() => {
    const scores = Array.from(scoresByCategory.values())
      .map(s => s.average)
      .filter(s => s > 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <Tabs
          value={perspective}
          onValueChange={v => setPerspective(v as typeof perspective)}
        >
          <TabsList>
        ...
        <div className="flex items-center gap-4">
          <Select
            ...
            onValueChange={v => setSelectedDomain(v as KpiDomain | "all")}
          >
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              <span>
                {KPI_CATEGORIES.find(c => c.id === selectedCategory)?.name}
              </span>
              <Button
                variant="ghost"
                size="sm"
                ...
                {(["cos", "sme", "customer"] as const).map(type => {
                  ...
                  return (
                    <div
                      ...
                      style={{ backgroundColor: getHeatMapColor(score) + "20" }}
                    >
                      <div
                    ...
                    {expertScores
                      ...
                      .map(es => {
                        const isOutlier = outliers.some(
                          o =>
                            o.expertId === es.expertId &&
                            o.categoryId === es.categoryId
                        );
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div
                                ...
                                style={{
                                  backgroundColor: getHeatMapColor(es.score),
                                }}
                              >
                                {es.score}
                              </div>
                    ...
                    return (
                      <>
                        <div className="flex gap-2">
                          ...
                          {cat.scoringCriteria.excellent}
                        </div>
                        <div className="flex gap-2">
                          <span className="text-lime-600 font-medium">
                            75-89:
                          </span>{" "}
                          {cat.scoringCriteria.good}
                        </div>
                        <div className="flex gap-2">
                          <span className="text-yellow-600 font-medium">
                            60-74:
                          </span>{" "}
                          {cat.scoringCriteria.adequate}
                        </div>
                        <div className="flex gap-2">
                          <span className="text-orange-600 font-medium">
                            40-59:
                          </span>{" "}
                          {cat.scoringCriteria.developing}
                        </div>
                        <div className="flex gap-2">
                          <span className="text-red-600 font-medium">
                            0-39:
                          </span>{" "}
                          {cat.scoringCriteria.critical}
                        </div>
                      </>
                    );
                  })()}
          ...
          <CardContent>
            <div className="space-y-3">
              {outliers.slice(0, 10).map((outlier, idx) => {
                const category = KPI_CATEGORIES.find(
                  c => c.id === outlier.categoryId
                );
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="text-center">
                        <div
                          className="text-2xl font-bold"
                          style={{ color: getHeatMapColor(outlier.score) }}
                        >
                          {outlier.score}
                        </div>
                        <div className="text-xs text-muted-foreground">
                          vs avg {Math.round(outlier.average)}
                        </div>
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="text-orange-600">
                        {outlier.score > outlier.average ? "+" : ""}
                        {Math.round(outlier.deviation)} deviation
                      </Badge>
                      <Button
                        variant="outline"
                        size="sm"
      ...
      <div className="flex items-center justify-center gap-6 text-sm">
        <div className="flex items-center gap-2">
          <div
            ...
            style={{ backgroundColor: getHeatMapColor(95) }}
          />
        ...
        <div className="flex items-center gap-2">
          <div
            className="w-4 h-4 rounded"
            style={{ backgroundColor: getHeatMapColor(80) }}
          />
        ...
        <div className="flex items-center gap-2">
          <div
            className="w-4 h-4 rounded"
            style={{ backgroundColor: getHeatMapColor(50) }}
          />
