...
export function ToolIntegrationsDashboard({
  filterPhase,
  filterCategory,
}: ToolIntegrationsDashboardProps) {
  ...
  const filteredTools = tools.filter(tool => {
    if (selectedPhase !== "all" && !tool.phases.includes(selectedPhase))
      return false;
  ...
  const handleSync = (toolId: string) => {
    ...
    setTimeout(() => {
      setTools(prev =>
        prev.map(t => (t.id === toolId ? { ...t, lastSync: new Date() } : t))
      );
      setSyncingId(null);
