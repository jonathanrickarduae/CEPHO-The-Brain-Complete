...
const SERVICE_DEFINITIONS: Record<
  string,
  Omit<IntegrationService, "status" | "email" | "hasApiKey" | "hasPassword">
> = {
  openai: {
    id: "openai",
    name: "OpenAI / ChatGPT",
...
export function IntegrationsStatusReal() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.integrations.getAll.useQuery(undefined, { retry: 0, retryDelay: 0 });
  ...
  const services: IntegrationService[] = Object.keys(SERVICE_DEFINITIONS).map(
    serviceId => {
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-lg bg-gradient-to-br ${categoryColors[service.category]} flex items-center justify-center`}
                >
                  {service.icon}
                </div>
                <div>
