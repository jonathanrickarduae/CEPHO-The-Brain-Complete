...
const STAGE_FOCUS: Record<
  ReviewStage,
  { icon: React.ReactNode; label: string; focus: string[] }
> = {
  strategy: {
...
export function JimShortExpert({
  context,
  projectName,
  stage: initialStage,
  onFeedback,
  ...
  const [selectedStage, setSelectedStage] = useState<ReviewStage>(
...
Project: ${projectName || "Not specified"}`;
  ...
  const askJim = trpc.chat.send.useMutation({
    onSuccess: data => {
      ...
      const score =
        message.includes("90") || message.includes("exceptional")
          ? 92
          : message.includes("85") || message.includes("strong")
            ? 82
            : message.includes("80")
              ? 78
              : message.includes("70") || message.includes("good foundation")
                ? 72
                : message.includes("60") || message.includes("promising")
                  ? 65
...
Remember: For every issue you identify, provide a specific "How to fix" suggestion.`;
  ...
  const getQuickQuestions = (stage: ReviewStage): string[] => {
    ...
    const stageQuestions: Record<ReviewStage, string[]> = {
      strategy: [
        "Is our competitive moat defensible?",
      ...
      product: [
        ...
        "What features should we cut?",
      ],
      team: [
        "Do we have the right people?",
        "What capability gaps hurt us most?",
      ],
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <CardDescription className="mt-1 text-base">
              Reviews every stage. Always provides fix suggestions. Goal: as
      ...
      <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <p
                className={cn(
                  "text-sm font-medium",
                  getVerdictInfo(feedback.verdict).color
                )}
              >
            ...
            <div className="p-3 bg-gray-800/50 rounded-lg border border-gray-700">
              ...
              <p className="text-xs text-muted-foreground">
                Every issue Jim identifies comes with a specific "how to fix it"
                recommendation.
              </p>
            </div>
          </div>
        ...
        <div className="p-4 bg-gradient-to-r from-amber-500/10 to-amber-600/5 border border-amber-500/20 rounded-lg">
          <p className="text-sm text-amber-200 italic leading-relaxed">
            "You might never make me completely happy, and that's fine. What
            matters is that you keep getting closer. Every piece of feedback I
            give comes with a fix. Use them. That's how good becomes great."
          </p>
          <div className="flex items-center gap-2 mt-2">
            ...
            <p className="text-xs text-amber-400/70">
              Jim Short — Reviews Every Stage, Fixes Every Issue
            </p>
          </div>
        </div>
      </CardContent>
