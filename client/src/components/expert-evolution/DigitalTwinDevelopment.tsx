...
import { MATURITY_MODEL } from "@/data/digital-twin-job-description.data";
...
export function DigitalTwinDevelopment() {
  const [activeTab, setActiveTab] = useState<
    "overview" | "maturity" | "learning" | "audit"
  >("overview");
  ...
  const [recentLearning] = useState<LearningEvent[]>([
    ...
    },
    {
      ...
      type: "passive",
      description:
        "Observed preference for bullet points over paragraphs in briefs",
    ...
    },
    {
      ...
      type: "expert",
      description:
        "Learned market analysis framework from McKinsey Chief of Staff",
  ...
  const calculateProgress = () => {
    ...
    const progress = {
      ...
      decisions: Math.min(
        100,
        (metrics.decisionsValidated / criteria.decisionsValidated) * 100
      ),
      accuracy: Math.min(
        100,
        (metrics.accuracyScore / criteria.accuracyScore) * 100
      ),
      trust: Math.min(
        100,
        (metrics.userTrustScore / criteria.userTrustScore) * 100
      ),
    };
    return Math.round(
      (progress.hours +
      ...
      <div className="p-6 border-b border-white/10">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-foreground/70">
                Track growth from Infant to Autonomous
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 bg-purple-500/20 rounded-lg border border-purple-500/30">
            ...
            <span className="text-purple-300 font-medium">
              Level {currentLevel}: {currentMaturity.name}
            </span>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          {[
            ...
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as "overview" | "learning" | "audit" | "maturity")}
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <span
                          className={`text-lg font-bold ${isUnlocked ? `text-${color}-400` : "text-foreground/60"}`}
                        >
                          {level.level}
                        </span>
                      </div>
                      <div>
                        <h3
                          className={`text-lg font-medium ${isUnlocked ? "text-white" : "text-foreground/60"}`}
                        >
                          {level.name}
                        </h3>
                        <p className="text-sm text-foreground/70">
                      ...
                      <div className="grid grid-cols-4 gap-4 text-center">
                        <div>
                          ...
                          <div className="text-sm text-foreground/70">
                            {level.unlockCriteria.hoursLogged}h
                          </div>
                        </div>
                        <div>
                          <div className="text-xs text-foreground/60">
                          ...
                          <div className="text-sm text-foreground/70">
                            {level.unlockCriteria.decisionsValidated}
                          </div>
                        </div>
                        <div>
                          <div className="text-xs text-foreground/60">
                          ...
                          <div className="text-sm text-foreground/70">
                            {level.unlockCriteria.accuracyScore}%
                          </div>
                        </div>
                        <div>
                          <div className="text-xs text-foreground/60">
                          ...
                          <div className="text-sm text-foreground/70">
                            {level.unlockCriteria.userTrustScore}
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    <method.icon
                      className={`w-4 h-4 text-${method.color}-400`}
                    />
                    <span className="text-sm text-foreground/70">
                      {method.label}
                    </span>
