...
const EXPERT_ROLES: ExpertRole[] = [
  ...
  },
  {
    id: "domain_expert",
    title: "Domain Expert",
    description: "Subject matter expertise for the project area",
  ...
  {
    id: "gcc_cultural",
    title: "GCC Cultural Advisor",
    description: "Local cultural awareness and regional insights",
    icon: <Globe className="w-5 h-5" />,
    category: "cultural",
    required: true,
  },
  {
    id: "western_perspective",
    title: "Western Perspective",
    description: "Global best practices and international standards",
    icon: <Globe className="w-5 h-5" />,
    category: "cultural",
    required: true,
  },
  {
    id: "gen_z_voice",
    title: "Gen Z Voice",
    description: "Fresh perspective from younger generation",
  ...
  },
  {
    ...
    description: "Current market expectations and fresh insights",
  ...
  },
  {
    ...
    description: "Implementation and process optimization",
  ...
  },
  {
    ...
    description: "Budget, ROI, and financial modeling",
  ...
  // Creative
  {
    id: "storyteller",
    title: "Storyteller",
    description: "Narrative development and compelling messaging",
  ...
  },
  {
    ...
    description: "Visual identity and presentation design",
  ...
  // Quality
  {
    id: "qa_reviewer",
    title: "QA Reviewer",
    description: "Quality assurance and standards compliance",
...
const TEAM_TEMPLATES: ExpertTeamTemplate[] = [
  {
    ...
    description:
      "Ideal for developing world class graduate and training programs",
  ...
  },
  {
    ...
    description:
      "Perfect for brand launches, campaigns, and market positioning",
...
export function ExpertTeamAssemblyWizard({
  projectType,
  onComplete,
  onBack,
}: ExpertTeamAssemblyWizardProps) {
  const [step, setStep] = useState<"template" | "customize" | "review">(
    "template"
  );
  const [selectedTemplate, setSelectedTemplate] =
    useState<ExpertTeamTemplate | null>(null);
  const [selectedRoles, setSelectedRoles] = useState<string[]>([]);
  const [selectedExperts, setSelectedExperts] = useState<SelectedExpert[]>([]);

  // Auto-select template based on project type
  const suggestedTemplate =
    TEAM_TEMPLATES.find(t =>
      t.projectTypes.some(pt =>
        projectType?.toLowerCase().includes(pt.toLowerCase())
      )
    ) || TEAM_TEMPLATES[0];
  ...
  const handleContinueToReview = () => {
    ...
    const experts: SelectedExpert[] = selectedRoles.map(roleId => ({
      ...
      perspective: roleId.includes("gcc")
        ? "gcc"
        : roleId.includes("western")
          ? "western"
          : "global",
    }));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-gray-800 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-foreground/70">
              Build your 12 person expert team for this project
            </p>
          </div>
        </div>
        ...
        {/* Progress Steps */}
        <div className="flex items-center gap-4 mb-8">
          {["Select Template", "Customize Team", "Review & Confirm"].map(
            (label, i) => {
              const stepNum = i + 1;
              const isActive =
                (step === "template" && stepNum === 1) ||
                (step === "customize" && stepNum === 2) ||
                (step === "review" && stepNum === 3);
              const isComplete =
                (step === "customize" && stepNum === 1) ||
              ...
              return (
                <div key={label} className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-sm ${isActive ? "text-white" : "text-foreground/70"}`}
                  >
                    {label}
                  </span>
                  {i < 2 && (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {TEAM_TEMPLATES.map(template => (
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-xl bg-gradient-to-r ${template.color} flex items-center justify-center mb-4`}
                  >
                    {template.icon}
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant="outline"
                      className="bg-white/5 text-foreground/80 border-white/20"
                    >
        ...
        {step === "customize" && selectedTemplate && (
          <div className="space-y-6">
            ...
            {(
              ...
              const categoryRoles = selectedTemplate.roles.filter(
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-foreground/70 text-sm">
                    Your {selectedExperts.length} person expert team is
                    assembled and ready to begin work on your project. Each
                    expert will contribute their unique perspective and
                    expertise.
                  </p>
                </div>
              </div>
