...
export function TaskProgressTracker({
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveTasks(prev =>
        prev.map(task => {
          if (task.stage === "complete" || task.stage === "blocked")
            return task;
  ...
  const inProgressTasks = activeTasks.filter(
    t => t.stage !== "complete" && t.stage !== "queued"
  );
  const nextCompletion =
    inProgressTasks.length > 0
      ? Math.min(
          ...inProgressTasks.map(t => t.estimatedMinutes - t.elapsedMinutes)
        )
      : 0;

  const totalEstimated = inProgressTasks.reduce(
    (sum, t) => sum + (t.estimatedMinutes - t.elapsedMinutes),
    0
  );
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      >
                        <StageIcon
                          className={`w-3 h-3 mr-1 ${task.stage === "researching" ? "animate-spin" : ""}`}
                        />
                ...
                <div className="mb-3">
                  <div className="flex items-center justify-between text-xs text-foreground/70 mb-1">
                    ...
                    <span>
                      {Math.round(task.elapsedMinutes)} /{" "}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <AssigneeIcon
                      className={`w-4 h-4 ${ASSIGNEE_CONFIG[task.assignedTo].color}`}
                    />
                    <span
                      className={`text-sm ${ASSIGNEE_CONFIG[task.assignedTo].color}`}
                    >
