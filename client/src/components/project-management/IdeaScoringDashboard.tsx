...
const MOCK_IDEAS: Idea[] = [
  {
    ...
    description:
      "Use AI to match graduates with mentors based on career goals, personality, and expertise alignment",
    ...
    scores: [
      {
        expertId: "1",
        expertName: "Strategy Lead",
        score: 90,
        comment: "High impact, differentiator",
        votedAt: new Date(),
      },
      {
        expertId: "2",
        expertName: "GCC Cultural",
        score: 80,
        comment: "Culturally appropriate",
        votedAt: new Date(),
      },
    ],
    status: "pending",
  ...
  },
  {
    ...
    description:
      "Gamified ranking system showing top performing graduates across cohorts",
    ...
    scores: [
      ...
      {
        expertId: "3",
        expertName: "Western Perspective",
        score: 80,
        comment: "Works well in Western contexts",
        votedAt: new Date(),
      },
    ],
    status: "pending",
  ...
  },
  {
    ...
    description:
      "Mandatory rotations across different GCC countries to build regional understanding",
    ...
    scores: [
      {
        ...
        expertName: "GCC Cultural",
        score: 100,
        comment: "Essential for regional success",
        votedAt: new Date(),
      },
      {
        expertId: "4",
        ...
        score: 70,
        comment: "Logistically complex but doable",
        votedAt: new Date(),
      },
    ],
    status: "pending",
  ...
  },
  {
    ...
    description:
      "Immersive VR experience for day one orientation and company culture introduction",
    ...
    scores: [
      {
        expertId: "5",
        expertName: "Gen Z Voice",
        score: 90,
        comment: "Would love this!",
        votedAt: new Date(),
      },
    ],
    status: "pending",
  ...
  },
  {
    ...
    description:
      "One week shadowing C-suite executives to understand strategic decision making",
...
export function IdeaScoringDashboard({
  ...
  onComplete,
  onBack,
}: IdeaScoringDashboardProps) {
  const [ideas, setIdeas] = useState<Idea[]>(MOCK_IDEAS);
  const [selectedIdea, setSelectedIdea] = useState<Idea | null>(null);
  const [filterStatus, setFilterStatus] = useState<Idea["status"] | "all">(
    "all"
  );
  ...
  const getAverageScore = (idea: Idea) => {
    ...
    return (
      idea.scores.reduce((sum, s) => sum + s.score, 0) / idea.scores.length
    );
  };
  ...
  const filteredIdeas = ideas
    ...
    .filter(
      idea => filterPriority === "all" || idea.priority === filterPriority
    )
    .sort((a, b) => {
      switch (sortBy) {
        case "score":
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-foreground/70">
                {projectName} • Leadership Team Review
              </p>
            </div>
          </div>
          <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <select
            value={filterStatus}
            onChange={e =>
              setFilterStatus(e.target.value as Idea["status"] | "all")
            }
          ...
          <select
            ...
            onChange={e =>
              setFilterPriority(e.target.value as Idea["priority"] | "all")
            }
            className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-white"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={sortBy}
              onChange={e =>
                setSortBy(e.target.value as "score" | "impact" | "recent")
              }
              className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-white"
            >
            ...
            {selectedIdea ? (
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-2 bg-white/10 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${selectedIdea.estimatedImpact * 10}%`,
                              }}
                            />
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-2 bg-white/10 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${selectedIdea.estimatedEffort * 10}%`,
                              }}
                            />
                          </div>
                          <span className="text-white font-medium text-sm">
                ...
                <Card className="bg-white/5 border-white/10">
                  <CardContent className="pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                      ...
                      <Button
                        onClick={() =>
                          updateIdeaStatus(selectedIdea.id, "deferred")
                        }
                      ...
                      <Button
                        onClick={() =>
                          updateIdeaStatus(selectedIdea.id, "rejected")
                        }
              ...
              <Card className="bg-white/5 border-white/10">
                <CardContent className="pt-6 text-center">
                  ...
                  <p className="text-foreground/70">
                    Select an idea to review and score
                  </p>
                </CardContent>
              </Card>
            )}
