          ...
          <div className="mb-3 p-3 rounded-lg bg-orange-500/10 border border-orange-500/30">
            ...
            <ul className="space-y-1">
              {statement.potentialIssues.map((issue, idx) => (
                <li
                  key={idx}
                  className="text-xs text-orange-400 flex items-start gap-2"
                >
        ...
        {expanded && (
          <div className="space-y-4 p-3 rounded-lg bg-secondary/30 border border-border">
                  ...
                  (() => {
                    ...
                    return (
                      <Button
                        ...
                        className={
                          statement.classification === cls
                            ? CLASSIFICATION_CONFIG[cls].bgColor
                            : ""
                        }
                      >
                        <Icon className="w-3 h-3 mr-1" />
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(
                  ["high", "medium", "low", "speculative"] as ConfidenceLevel[]
                ).map(conf => (
                  <Button
                    key={conf}
                    variant={
                      statement.confidence === conf ? "default" : "outline"
                    }
                    size="sm"
                    onClick={() => onSetConfidence(statement.id, conf)}
                    className={
                      statement.confidence === conf
                        ? CONFIDENCE_INDICATORS[conf].bgColor
                        : ""
                    }
                  >
                    {CONFIDENCE_INDICATORS[conf].icon}{" "}
                    {CONFIDENCE_INDICATORS[conf].label}
                  </Button>
                ))}
              </div>
            </div>
...
export function BatchVerificationPanel({
  ...
  const issueCount = statements.filter(
    s => s.potentialIssues.length > 0
  ).length;
        ...
        <div>
          <div className="flex items-center justify-between text-sm mb-2">
            ...
            <span className="font-medium">
              {approvedCount} / {statements.length}
            </span>
          </div>
    ...
    <Card className="bg-purple-500/5 border-purple-500/30">
      ...
      <CardContent className="space-y-4">
        <p className="text-sm text-muted-foreground">
          Request {expertName} to validate their {statements.length} statements
          by explicitly classifying each as fact, analysis, opinion, or
          speculation with confidence levels.
        </p>
...
export function TruthVerificationSummary({
  statements,
}: TruthVerificationSummaryProps) {
  const byClassification = {
    ...
    speculation: statements.filter(s => s.classification === "speculation")
      .length,
  ...
  const withIssues = statements.filter(
    s => s.potentialIssues.length > 0
  ).length;
  const needingSources = statements.filter(
    s => s.requiredSources.length > 0
  ).length;
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
            {(
              ...
              <div
                key={cls}
                className={`text-center p-2 rounded-lg ${CLASSIFICATION_CONFIG[cls].bgColor}`}
              >
                <div
                  className={`text-lg font-bold ${CLASSIFICATION_CONFIG[cls].color}`}
                >
                  {count}
                </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
            {(Object.entries(byConfidence) as [ConfidenceLevel, number][]).map(
              ([conf, count]) => (
                <div
                  key={conf}
                  className={`text-center p-2 rounded-lg ${CONFIDENCE_INDICATORS[conf].bgColor}`}
                >
                  <div
                    className={`text-lg font-bold ${CONFIDENCE_INDICATORS[conf].color}`}
                  >
                    {count}
                  </div>
                  <div className="text-xs text-muted-foreground">
