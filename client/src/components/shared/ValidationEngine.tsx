      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        ...
        <div>
          ...
          <p className="text-white font-medium">
            ...
            {report.flaggedClaims > 0 && (
              <span className="text-orange-400">
                {" "}
                | {report.flaggedClaims} flagged
              </span>
            )}
            {report.rejectedClaims > 0 && (
              <span className="text-red-400">
                {" "}
                | {report.rejectedClaims} rejected
              </span>
            )}
          </p>
        </div>
    ...
    <div className="flex items-start gap-3 p-3 bg-gray-800/50 rounded-lg border border-gray-700">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 text-xs text-foreground/70">
          <span>
            Verified: {new Date(source.accessDate).toLocaleDateString("en-GB")}
          </span>
          <span
            className={source.isAccessible ? "text-green-400" : "text-red-400"}
          >
            {source.isAccessible ? "✓ Active" : "✗ Inactive"}
          </span>
          <span
            className={
              source.authorityScore === "high"
                ? "text-green-400"
                : source.authorityScore === "medium"
                  ? "text-yellow-400"
                  : "text-orange-400"
            }
          >
            Authority:{" "}
            {source.authorityScore.charAt(0).toUpperCase() +
              source.authorityScore.slice(1)}
          </span>
        </div>
      </div>
    </div>
          ...
          {claim.crossValidation && (
            <div className="bg-gray-800/50 rounded-lg p-3">
              ...
              <div className="text-sm text-foreground/70">
                ...
                <p>
                  Agreement:{" "}
                  {claim.crossValidation.agrees ? "✓ Confirmed" : "✗ Disputed"}
                </p>
                <p>Confidence: {claim.crossValidation.confidence}%</p>
                {claim.crossValidation.notes && (
...
export function ValidationSummary({ report }: { report: ValidationReport }) {
  const metrics = [
    ...
    {
      label: "Verified",
      value: `${report.verifiedClaims} (${Math.round((report.verifiedClaims / report.totalClaims) * 100)}%)`,
      className: "text-green-400",
    },
    {
      label: "Flagged for Review",
      value: `${report.flaggedClaims} (${Math.round((report.flaggedClaims / report.totalClaims) * 100)}%)`,
      className: "text-orange-400",
    },
    {
      label: "Rejected",
      value: `${report.rejectedClaims} (${Math.round((report.rejectedClaims / report.totalClaims) * 100)}%)`,
      className: "text-red-400",
    },
    { label: "Cross-Validation Engine", value: report.crossValidationEngine },
    {
      label: "Human Review Required",
      value: report.humanReviewRequired ? "Yes" : "No",
    },
  ];
      ...
      {!report ? (
        <div className="text-center py-12">
          ...
          <p className="text-foreground/70 mb-6 max-w-md mx-auto">
            Run comprehensive validation to cross-check all claims, verify
            sources, and ensure zero hallucinations.
          </p>
          <Button
