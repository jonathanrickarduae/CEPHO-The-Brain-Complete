  ...
  const commands: VoiceCommand[] = [
    ...
    },
    {
      ...
      action: () => {
        ...
        speak("Opening your daily brief");
      },
    ...
    // Action commands
    {
      ...
      action: transcript => {
        const taskMatch = transcript.match(
          /(?:create|new|add) task (?:called |named |titled )?(.+)/i
        );
        const taskName = taskMatch?.[1] || "New task";
        speak(`Creating task: ${taskName}`);
        setLocation(
          "/workflow?action=new-task&name=" + encodeURIComponent(taskName)
        );
      },
      ...
      action: transcript => {
        ...
        setLocation(
          "/digital-twin?message=" +
            encodeURIComponent(
              `Help me draft an email${recipient ? ` to ${recipient}` : ""}`
            )
        );
      },
    ...
    },
    {
      ...
      action: _transcript => {
        ...
        setLocation(
          "/digital-twin?message=" +
            encodeURIComponent("Help me schedule a meeting")
        );
      },
  ...
  const processCommand = useCallback(
    (text: string) => {
      ...
      if (isHandsFree && !wakeWordDetected) {
        if (
          lowerText.includes("hey brain") ||
          lowerText.includes("okay brain")
        ) {
