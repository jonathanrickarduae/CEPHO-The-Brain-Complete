...
const INITIAL_TASKS: ProposedTask[] = [
  {
    id: "1",
    icon: Mail,
    task: "Draft response to investor inquiry",
    approved: true,
  },
  {
    id: "2",
    icon: FileText,
    task: "Review contract amendments",
    approved: true,
  },
  {
    id: "3",
    icon: Calendar,
    task: "Prep notes for your 3pm call",
    approved: true,
  },
];

export function TwinBreakApproval({
  onApprove,
  onCancel,
}: {
  onApprove?: () => void;
  ...
  const toggleTask = (id: string) => {
    setTasks(
      tasks.map(t => (t.id === id ? { ...t, approved: !t.approved } : t))
    );
  };
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <p className="font-semibold text-foreground">
            While you take 15 mins, I'll:
          </p>
          <p className="text-xs text-muted-foreground">
            {approvedCount} tasks ready to go
          </p>
        </div>
      </div>
        ...
        {tasks.map(task => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`text-sm flex-1 ${task.approved ? "text-foreground" : "text-muted-foreground line-through"}`}
              >
                {task.task}
              </span>
            </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground">
          I have 3 tasks ready while you're away
        </p>
      </div>
