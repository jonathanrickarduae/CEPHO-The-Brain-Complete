...
const MOCK_INSIGHTS: MarketInsight[] = [
  {
    ...
    description:
      "Analysis of competitor pricing shows a gap in the £500-2000/month range for mid-market companies.",
  ...
  },
  {
    ...
    description:
      "Competitor announced 30% price cut for enterprise tier, potentially impacting our pipeline.",
  ...
  },
  {
    ...
    description:
      "Industry moving towards specialized AI for specific verticals rather than general-purpose tools.",
...
const MOCK_POSITIONS: CompetitivePosition[] = [
  ...
  },
  {
    ...
    yourScore: 90,
    avgCompetitorScore: 68,
    leader: "You",
  },
  {
    category: "Brand Recognition",
    yourScore: 45,
    avgCompetitorScore: 75,
    leader: "TechCorp AI",
  },
  {
    category: "Integration Ecosystem",
    yourScore: 60,
    avgCompetitorScore: 70,
    leader: "TechCorp AI",
  },
];
...
export function CompetitiveIntelligence() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "competitors" | "insights" | "position"
  >("overview");
      ...
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Card className="bg-card/60 border-border">
            <CardContent className="p-4 text-center">
              ...
              <div className="flex items-center justify-center gap-1 mt-1 text-sm text-yellow-400">
                ...
                {competitors.filter(c => c.threatLevel === "high").length} high
                threat
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-card/60 border-border">
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-foreground">
                {positions.filter(p => p.leader === "You").length}/
                {positions.length}
              </div>
              <div className="text-sm text-muted-foreground">
                Categories Led
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        <span
                          className={
                            competitor.marketShareChange > 0
                              ? "text-green-400"
                              : "text-red-400"
                          }
                        >
                          {competitor.marketShareChange > 0 ? "+" : ""}
                          {competitor.marketShareChange}%
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            {positions.map(pos => (
              <div key={pos.category} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Badge
                    variant={pos.leader === "You" ? "default" : "secondary"}
                    className="text-xs"
                  >
                    Leader: {pos.leader}
                  </Badge>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Progress
                      value={pos.avgCompetitorScore}
                      className="flex-1 h-2 [&>div]:bg-gray-500"
                    />
                    <span className="text-xs font-medium text-muted-foreground w-8">
                      {pos.avgCompetitorScore}
                    </span>
                  </div>
                </div>
              </div>
