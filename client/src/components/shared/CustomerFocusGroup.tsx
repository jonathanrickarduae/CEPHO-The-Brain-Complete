...
export function CustomerFocusGroup({
  ...
  const [newQuestionType, setNewQuestionType] =
    useState<SurveyQuestion["type"]>("rating");
  ...
  const industries = useMemo(() => {
    const uniqueIndustries = Array.from(
      new Set(CUSTOMER_PERSONAS.map(p => p.industry))
    );
    ...
    for (const persona of selectedPersonas) {
      for (const question of surveyQuestions) {
        ...
        if (question.type === "rating") {
          ...
          const baseScore =
            persona.techComfort * 1.5 +
            (persona.riskTolerance === "aggressive"
              ? 2
              : persona.riskTolerance === "moderate"
                ? 1
                : 0);
          response = Math.min(
            10,
            Math.max(1, Math.round(baseScore + (Math.random() * 3 - 1.5)))
          );
          ...
          response =
            persona.buyingMotivation === "price"
              ? "Low"
              : persona.buyingMotivation === "status"
                ? "Premium"
                : "Mid-range";
  ...
  const aggregateStats = useMemo(() => {
    ...
    const ratingResponses = simulatedResponses.filter(
      r => typeof r.response === "number"
    );
    const avgRating =
    ...
    const priceResponses = simulatedResponses.filter(
      r =>
        typeof r.response === "string" &&
        ["Low", "Mid-range", "Premium"].includes(r.response)
    );
    const priceCounts = {
      Low: priceResponses.filter(r => r.response === "Low").length,
      "Mid-range": priceResponses.filter(r => r.response === "Mid-range")
        .length,
    ...
    return {
      avgRating,
      priceCounts,
      totalResponses: simulatedResponses.length,
    };
  }, [simulatedResponses]);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select customer personas to validate {productName}. Currently{" "}
            {CUSTOMER_PERSONAS.length} personas available,{" "}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="browse" className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4 bg-muted/50 rounded-lg">
                ...
                <div className="flex gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => selectRandomPersonas(10)}
                  >
                    Random 10
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => selectRandomPersonas(25)}
                  >
                    Random 25
                  </Button>
                </div>
              ...
              <div className="text-sm text-muted-foreground mb-2">
                Showing {filteredPersonas.length} of {CUSTOMER_PERSONAS.length}{" "}
                personas
              </div>
                    ...
                    >
                      <CardContent className="p-4">
                        ...
                        <div className="flex gap-2 mt-2">
                          <Badge
                            className={`text-xs ${getIncomeColor(persona.incomeLevel)}`}
                          >
            ...
            <TabsContent value="selected" className="space-y-4">
              {selectedPersonas.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">
                  No personas selected. Browse and select personas to build your
                  focus group.
                </div>
              ) : (
                <>
                  <div className="flex justify-between items-center">
                    <h3 className="font-semibold">
                      Selected Focus Group ({selectedPersonas.length} personas)
                    </h3>
                    <Button
                      variant="outline"
                      size="sm"
                        ...
                        <CardContent className="p-4">
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground mt-1">
                                {persona.age}yo | {persona.nationality} |{" "}
                                {persona.location}
                              </p>
                            </div>
                            <Button
                          ...
                          <div className="mt-3 space-y-2">
                            ...
                            <div className="flex gap-2 text-xs">
                              <span>
                                Decision Style:{" "}
                                <strong>{persona.decisionStyle}</strong>
                              </span>
                              <span>
                                Risk: <strong>{persona.riskTolerance}</strong>
                              </span>
                              <span>
                                Motivation:{" "}
                                <strong>{persona.buyingMotivation}</strong>
                              </span>
                            </div>
                          </div>
                        </CardContent>
            ...
            <TabsContent value="survey" className="space-y-4">
              ...
              <div className="flex justify-end">
                ...
                >
                  {isSimulating
                    ? "Simulating..."
                    : `Run Survey (${selectedPersonas.length} personas)`}
                </Button>
              </div>
            </TabsContent>
            ...
            <TabsContent value="results" className="space-y-4">
              {simulatedResponses.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">
                  No survey results yet. Build a survey and run it with selected
                  personas.
                </div>
              ) : (
                <>
                      ...
                      <Card>
                        <CardContent className="p-4">
                          ...
                          <div className="text-3xl font-bold">
                            {aggregateStats.avgRating.toFixed(1)}/10
                          </div>
                        </CardContent>
                      </Card>
                      <Card>
                        <CardContent className="p-4">
                          ...
                          <div className="flex gap-2 mt-2">
                            <Badge variant="outline">
                              Low: {aggregateStats.priceCounts["Low"]}
                            </Badge>
                            <Badge variant="secondary">
                              Mid: {aggregateStats.priceCounts["Mid-range"]}
                            </Badge>
                            <Badge>
                              Premium: {aggregateStats.priceCounts["Premium"]}
                            </Badge>
                          </div>
                        </CardContent>
                      </Card>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                              {simulatedResponses
                                ...
                                .map(response => {
                                  ...
                                  return (
                                    <div
                                      key={`${response.personaId}-${response.questionId}`}
