...
export function ActivityIndicator({
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      {label && (
        <span
          className={`text-xs ${isActive ? "text-green-400" : "text-muted-foreground"}`}
        >
          {label}
        </span>
      )}
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="relative">
        {config.pulse && (
          <span
            className={`absolute inset-0 w-2 h-2 ${config.color} rounded-full animate-ping opacity-75`}
          />
        )}
        <span
          className={`relative w-2 h-2 ${config.color} rounded-full block`}
        />
      </div>
      {showLabel && (
