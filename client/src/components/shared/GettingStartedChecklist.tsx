...
export function GettingStartedChecklist({
  ...
  const toggleItem = (id: string) => {
    setItemsData(prev => {
      ...
      if (
        newCompletedCount === newItems.length &&
        completedCount < newItems.length
      ) {
        showAchievement({
      ...
      <div className="space-y-2">
        {items.map(item => (
          ...
          >
            {/* Checkbox */}
            <div
              className={`flex-shrink-0 ${item.completed ? "text-green-400" : "text-muted-foreground"}`}
            >
              {item.completed ? (
            ...
            <div className="flex-1 text-left">
              <p
                className={`font-medium ${item.completed ? "text-green-400 line-through" : "text-foreground"}`}
              >
                {item.title}
              </p>
