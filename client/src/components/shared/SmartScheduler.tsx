...
function generateOptimalSchedule(
  ...
  for (const task of sortedTasks) {
    ...
    const hoursSinceStart =
      (currentTime.getTime() - workStart.getTime()) / (60 * 60000);
    if (
      hoursSinceStart > 0 &&
      hoursSinceStart % 2 < 0.5 &&
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                onChange={e =>
                  setPreferences(p => ({
                    ...p,
                    peakHoursStart: parseInt(e.target.value),
                  }))
                }
                className="bg-secondary/50 border border-white/10 rounded px-2 py-1 text-xs"
              >
              ...
              <select
                ...
                onChange={e =>
                  setPreferences(p => ({
                    ...p,
                    peakHoursEnd: parseInt(e.target.value),
                  }))
                }
                className="bg-secondary/50 border border-white/10 rounded px-2 py-1 text-xs"
              >
          ...
          <div className="col-span-2 flex flex-wrap gap-3">
            <label className="flex items-center gap-2 text-xs">
              <input
                ...
                onChange={e =>
                  setPreferences(p => ({
                    ...p,
                    includeMorningRoutine: e.target.checked,
                  }))
                }
                className="rounded"
              />
            ...
            <label className="flex items-center gap-2 text-xs">
              <input
                ...
                onChange={e =>
                  setPreferences(p => ({ ...p, addBuffers: e.target.checked }))
                }
                className="rounded"
              />
              Add buffers
        ...
        <div className="flex items-center gap-4 text-xs">
          <div className="flex items-center gap-1">
            ...
            <span className="text-muted-foreground">
              {Math.round(totalFocusTime / 60)}h focus time
            </span>
          </div>
          <div className="flex items-center gap-1">
      ...
      <div className="p-4 max-h-96 overflow-y-auto">
        {schedule.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-xs">
              Add tasks to generate an optimized schedule
            </p>
          </div>
        ) : (
      ...
      <div className="px-4 py-3 border-t border-white/10 bg-secondary/20">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>
              Peak: {preferences.peakHoursStart}:00 - {preferences.peakHoursEnd}
              :00
            </span>
          </div>
          <Button
