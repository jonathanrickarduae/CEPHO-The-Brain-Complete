...
export function GovernanceSettings() {
  ...
  const [filterMode, setFilterMode] = useState<"all" | "allowed" | "blocked">(
    "all"
  );
  ...
  const handleModeToggle = () => {
    ...
    toast.success(
      `Switched to ${newMode === "omni" ? "Everything" : "Governed"} mode`
    );
  };
  ...
  const allowedCount = REAL_INTEGRATIONS.filter(i =>
    mode === "omni" ? true : i.governedAllowed
  ).length;
  const blockedCount =
    mode === "governed" ? REAL_INTEGRATIONS.length - allowedCount : 0;
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          ...
          >
            <div className="flex items-start gap-3">
              <Info
                className={`w-5 h-5 mt-0.5 ${mode === "governed" ? "text-emerald-400" : "text-amber-400"}`}
              />
              <div className="flex-1">
                <p
                  className={`font-medium ${mode === "governed" ? "text-emerald-400" : "text-amber-400"}`}
                >
                  {mode === "governed"
                    ? "Governed Mode Active"
                    : "Everything Mode Active"}
                </p>
        ...
        <div className="flex gap-2">
          ...
          {mode === "governed" && (
            <Button
              ...
              className={
                filterMode === "blocked" ? "bg-red-600 hover:bg-red-700" : ""
              }
            >
      ...
      <div className="space-y-2">
        {filteredIntegrations.map(integration => {
          const isAllowed =
            mode === "omni" ? true : integration.governedAllowed;
