  ...
  },
  {
    ...
    guidance: {
      ...
      "financial-projections":
        "Detail capital requirements and unit economics.",
...
export function BusinessPlanReview({
  ...
  const [teamSelectionMode, setTeamSelectionMode] =
    useState<TeamSelectionMode>("chief-of-staff");
  const [selectedExperts, setSelectedExperts] = useState<string[]>(
    REVIEW_EXPERTS.map(e => e.id)
  );
  ...
  const [followUpExpert, setFollowUpExpert] = useState<ExpertInsight | null>(
    null
  );
  const [followUpSection, setFollowUpSection] = useState<string>("");
  const [followUpQuestion, setFollowUpQuestion] = useState("");
  const [followUpHistory, setFollowUpHistory] = useState<
    Array<{ question: string; answer: string; expertName: string }>
  >([]);
  ...
  const [collaborativeSessionId, setCollaborativeSessionId] = useState<
    number | null
  >(null);
  ...
  const completedSections = Array.from(sectionReviews.values()).filter(
  ...
  const analyzeSectionMutation =
    trpc.businessPlanReview.analyzeSectionWithAllExperts.useMutation();

  // Chief of Staff team selection mutation
  const selectTeamMutation =
    trpc.businessPlanReview.selectExpertTeam.useMutation();
  ...
  const generatePdfMutation =
    trpc.businessPlanReview.generateReportMarkdown.useMutation();

  // Collaborative review mutations
  const createSessionMutation =
    trpc.collaborativeReview.createSession.useMutation();
  ...
  const reviewNextSection = async (
    index: number,
    reviews: Map<string, SectionReview>
  ) => {
    ...
    try {
      ...
      if (
        ...
        if (fullText.trim()) {
          ...
          toast.info(
            `PDF uploaded but text extraction limited - may be image-based`
          );
        }
  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      ...
      for (const review of reviews) {
        ...
        const section = BUSINESS_PLAN_SECTIONS.find(
          s => s.id === review.sectionId
        );
        if (!section) continue;
  ...
  const handleSaveVersion = async () => {
    if (completedSections === 0) {
      toast.error(
        "Complete at least one section review before saving a version"
      );
      return;
    }
    ...
    try {
      ...
      const result = await saveVersionMutation.mutateAsync({
        projectName,
        versionLabel:
          versionLabel || `Review ${new Date().toLocaleDateString("en-GB")}`,
        overallScore,
        sectionScores,
        reviewData: reviews,
  ...
  const handleSectionDocumentUpload = async (
    sectionId: string,
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const file = event.target.files?.[0];
  ...
  const handleExportPdf = async () => {
    ...
    try {
      const sectionReviewsArray = Array.from(sectionReviews.entries()).map(
        ([sectionId, review]) => {
          const sectionDef = BUSINESS_PLAN_SECTIONS.find(
            s => s.id === sectionId
          );
          return {
            sectionId,
  ...
  const currentTemplate = selectedTemplate
    ? BUSINESS_TEMPLATES.find(t => t.id === selectedTemplate)
    : null;
      ...
      {showTemplateSelector && completedSections === 0 && !isReviewing && (
        <div className="shrink-0 p-6 border-b border-white/10 bg-gradient-to-r from-purple-500/10 to-pink-500/10">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Choose a template to get tailored analysis with industry-specific
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {!isReviewing && !review?.status && (
                              <div className="mt-2 flex items-center gap-2">
                                <input
                                  ...
                                  onChange={e =>
                                    handleSectionDocumentUpload(section.id, e)
                                  }
                            ...
                            {review?.expertInsights &&
                              review.expertInsights.length > 0 && (
          ...
          <TabsContent value="experts" className="flex-1 overflow-hidden p-4">
            <ScrollArea className="h-full">
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-2">
                      {teamSelectionResult.teamComposition.map((tc, i) => {
                        const expert = REVIEW_EXPERTS.find(
                          e => e.id === tc.expertId
                        );
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {REVIEW_EXPERTS.filter(e => selectedExperts.includes(e.id)).map(
                  expert => {
                    // Count how many sections this expert reviewed
                    const sectionsReviewed = Array.from(
                      sectionReviews.values()
                    ).filter(r =>
                      r.expertInsights.some(i => i.expertId === expert.id)
                    ).length;
                    const teamRole = teamSelectionResult?.teamComposition.find(
                      tc => tc.expertId === expert.id
                    );
            ...
            <ScrollArea className="h-full">
              {completedSections === BUSINESS_PLAN_SECTIONS.length ? (
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            Based on {REVIEW_EXPERTS.length} expert reviews
                            across {BUSINESS_PLAN_SECTIONS.length} sections
                          </p>
                        </div>
                        <div className="text-right">
                          <div className="text-4xl font-bold text-green-400">
                            {Math.round(
                              Array.from(sectionReviews.values()).reduce(
                                (sum, r) => sum + (r.overallScore || 0),
                                0
                              ) / BUSINESS_PLAN_SECTIONS.length
                            )}
                            %
                          </div>
                ...
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {completedSections} of {BUSINESS_PLAN_SECTIONS.length}{" "}
                    sections reviewed
                  </p>
                </div>
              )}
      ...
      <Dialog open={showExpertSelection} onOpenChange={setShowExpertSelection}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
          ...
          <ScrollArea className="flex-1 pr-4">
            <p className="text-sm text-muted-foreground mb-4">
              Choose which experts should review your business plan. Each expert
              brings unique perspectives based on their specialty.
            </p>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-white/10">
            <p className="text-sm text-muted-foreground">
              {selectedExperts.length} of {REVIEW_EXPERTS.length} experts
              selected
            </p>
            <div className="flex gap-2">
              <Button
                variant="outline"
                onClick={() =>
                  setSelectedExperts(REVIEW_EXPERTS.map(e => e.id))
                }
              >
                Select All
              </Button>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
          ...
          <ScrollArea className="flex-1 pr-4">
            {versionsQuery.isLoading ? (
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Score: {version.overallScore || "N/A"}% •{" "}
                            {version.teamSelectionMode === "chief-of-staff"
                              ? "Auto Team"
                              : "Manual Team"}
                          </p>
                        </div>
                          ...
                          <div className="mt-4 pt-4 border-t border-white/10">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              {Object.entries(
                                ...
                                const currentScore =
                                  sectionReviews.get(sectionId)?.overallScore ||
                                  0;
              ...
              <div className="text-center py-8 text-muted-foreground">
                No versions saved yet. Complete a review and click "Save
                Version" to track changes over time.
              </div>
            )}
          </ScrollArea>
        </DialogContent>
            ...
            <div className="mb-4 p-3 bg-white/5 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-lg">
                  {
                    REVIEW_EXPERTS.find(e => e.id === followUpExpert.expertId)
                      ?.avatar
                  }
                </span>
                <span className="font-medium text-foreground">
                  {followUpExpert.expertName}
                </span>
              </div>
              <p className="text-sm text-muted-foreground italic">
                "{followUpExpert.insight.slice(0, 200)}..."
              </p>
            </div>
          )}
      ...
      <Dialog open={showInviteDialog} onOpenChange={setShowInviteDialog}>
        <DialogContent className="bg-slate-900 border-white/10">
          ...
          {sessionQuery.data?.participants &&
            sessionQuery.data.participants.length > 0 && (
              <div className="mt-4 pt-4 border-t border-white/10">
                <h4 className="text-sm font-medium text-foreground mb-2">
