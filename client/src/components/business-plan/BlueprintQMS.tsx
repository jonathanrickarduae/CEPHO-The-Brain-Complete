...
export function BlueprintQMS({
  ...
  const determineSeverity = (
    ...
    if (
      ["valueProposition", "objectives", "targetAudience"].some(f =>
        field.includes(f)
      )
    ) {
      return "high";
    ...
    // Medium severity
    if (
      ["businessInfo", "revenueModel", "keywords"].some(f => field.includes(f))
    ) {
      return "medium";
    }
    return "low";
  ...
  const isAutoUpdateable = (field: string): boolean => {
    ...
    return ["companyName", "industry", "description"].some(f =>
      field.includes(f)
    );
  };
  ...
  const requiresReview = (field: string, blueprintType: string): boolean => {
    ...
    if (
      ["valueProposition", "objectives", "strategy"].some(f =>
        field.includes(f)
      )
    ) {
      return true;
  ...
  const getBlueprintLabel = (type: LinkedBlueprint["type"]) => {
    switch (type) {
      case "presentation":
        return "Presentation Blueprint";
      case "social_media":
        return "Social Media Blueprint";
      case "financial_model":
        return "Financial Model Blueprint";
      case "marketing":
        return "Marketing Blueprint";
      case "operations":
  ...
  const getStatusColor = (status: LinkedBlueprint["status"]) => {
    switch (status) {
      ...
      default:
        return "text-foreground/70 border-white/20 bg-white/5";
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-foreground/70">
              Blueprint interconnections & cascading updates
            </p>
          </div>
        </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
        ...
        <div className="relative">
          ...
          <div className="flex justify-center mb-8">
            <div className="relative">
              <div
                className={`w-32 h-32 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-fuchsia-500/20 border-2 ${
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge
                        variant="outline"
                        className="text-[10px] border-white/20 text-foreground/70"
                      >
                        {change.changedBy === "user" && (
                          <User className="w-2 h-2 mr-1" />
                        )}
                        {change.changedBy === "sme" && (
                          <Sparkles className="w-2 h-2 mr-1" />
                        )}
                        {change.changedBy === "digital_twin" && (
          ...
          {selectedChanges.length > 0 && cascadeImpacts.length > 0 && (
            <div className="mt-4 p-4 bg-orange-500/10 rounded-xl border border-orange-500/20">
              ...
              <p className="text-xs text-orange-200/70 mb-3">
                These changes will affect {cascadeImpacts.length} downstream
                blueprint(s)
              </p>
