...
export function GenesisBlueprintWizard({
  ...
  const [digitalTwinMessages, setDigitalTwinMessages] = useState<
    DigitalTwinMessage[]
  >([]);
  ...
  const sectionQuestions = genesisWizardQuestions.filter(
  ...
  useEffect(() => {
    if (currentQuestion?.smeRelevant) {
      ...
      currentQuestion.smeRelevant.forEach(smeType => {
        ...
        const matchingExperts = allExperts
          .filter(
            (e: AIExpert) =>
  ...
  useEffect(() => {
    if (
      currentQuestion &&
      !digitalTwinMessages.find(m => m.id === `q-${currentQuestion.id}`)
    ) {
      // Add contextual help
  ...
  const handleAnswer = (value: string | string[] | number) => {
    ...
    if (activeSMEs.length > 0 && Math.random() > 0.6) {
      const contributingSME =
        activeSMEs[Math.floor(Math.random() * activeSMEs.length)];
      setTimeout(() => {
        setActiveSMEs(prev =>
          prev.map(sme =>
            sme.id === contributingSME.id
              ? {
                  ...sme,
                  status: "contributing",
                  contribution: getSmEContribution(currentQuestion, value),
                }
              : sme
          )
        );
  ...
  const getSmEContribution = (
    ...
    return questionContributions[
      Math.floor(Math.random() * questionContributions.length)
    ];
  };
  ...
  const handleComplete = () => {
    ...
    const blueprint: Partial<GenesisBlueprint> = {
      ...
      decisions: Object.entries(answers).map(([key, value]) => ({
        ...
        timestamp: new Date(),
        section:
          genesisWizardQuestions.find(q => q.id === key)?.section || "unknown",
        question:
          genesisWizardQuestions.find(q => q.id === key)?.question || key,
      ...
      linkedBlueprints: [
        {
          ...
          status: "not_started",
          inheritedFields: ["businessInfo", "valueProposition", "objectives"],
        },
        {
          id: "social_media",
          type: "social_media",
          ...
          status: "not_started",
          inheritedFields: ["businessInfo", "targetAudience", "keywords"],
        },
        {
          id: "financial_model",
          ...
          status: "not_started",
          inheritedFields: ["revenueModel", "objectives", "timeline"],
        },
      ],
    };
  ...
  const mapStage = (
    answer: string
  ): GenesisBlueprint["businessInfo"]["stage"] => {
  ...
  const mapTimeframe = (
    answer: string
  ): GenesisBlueprint["objectives"]["timeframe"] => {
  ...
  const mapRevenueType = (
    answer: string
  ): GenesisBlueprint["revenueModel"]["type"] => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTimeout(() => {
      addDigitalTwinMessage({
        ...
        type: "suggestion",
        message:
          "I found company information, market data, and financial projections. I've pre-filled some sections - you can review and adjust as needed.",
        timestamp: new Date(),
      });
    }, 3000);
  };
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-white/10">
                    <Button
                      ...
                      disabled={
                        currentSection === "entry" && currentQuestionIndex === 0
                      }
                    ...
                    {activeCorporates.map(name => {
                      ...
                      return corp ? (
                        <Badge
                          key={name}
                          variant="outline"
                          className="border-fuchsia-500/30 text-fuchsia-300"
                        >
                          {corp.logo} {corp.name}
                        </Badge>
                      ) : null;
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-background border border-white/10 rounded-2xl p-8 max-w-md text-center">
            ...
            <p className="text-foreground/70 mb-4">
              Compiling insights, engaging experts, and preparing your strategic
              foundation...
            </p>
