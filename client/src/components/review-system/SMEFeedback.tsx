  ...
  return (
    <div className="bg-gray-900/80 backdrop-blur-sm border border-border/50 rounded-2xl p-6 max-w-md mx-auto">
      ...
      <div className="flex gap-3 mb-6">
        <button
          onClick={() =>
            setQuickFeedback(quickFeedback === "helpful" ? null : "helpful")
          }
        ...
        <button
          onClick={() => {
            setQuickFeedback(
              quickFeedback === "not_helpful" ? null : "not_helpful"
            );
...
export function ExpertPerformanceCard({
  ...
  onAction: (
    action: "enhance" | "retrain" | "demote" | "promote" | "suspend"
  ) => void;
}) {
  const getTrendIcon = () => {
    switch (expert.trend) {
      ...
      default:
        return <Minus className="w-4 h-4 text-foreground/70" />;
    }
  };
    ...
    <div className="bg-gray-900/50 border border-border/50 rounded-xl p-4">
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        <div>
          ...
          <div
            className={`text-xl font-bold ${getRatingColor(expert.averageRating)}`}
          >
