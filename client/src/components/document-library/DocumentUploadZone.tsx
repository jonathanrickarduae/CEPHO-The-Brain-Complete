...
export function DocumentUploadZone({
  ...
  const getFileIcon = (type: string) => {
    switch (type) {
      ...
      default:
        return <File className="w-5 h-5 text-foreground/70" />;
    }
  };
  ...
  const simulateExtraction = useCallback((doc: UploadedDocument) => {
    ...
    const mockExtractions: Record<string, ExtractedData> = {
      pitch_deck: {
        ...
        stage: "Seed",
        description:
          "AI-powered platform for business intelligence and strategic planning.",
        objectives: [
          "Raise Series A",
          "Expand to US market",
      ...
      business_plan: {
        ...
        industry: "Technology",
        description:
          "Comprehensive business plan with detailed market analysis.",
  ...
  const processFile = useCallback(
    async (file: File) => {
      ...
      for (let i = 0; i <= 100; i += 10) {
        ...
        setDocuments(prev =>
          prev.map(d => (d.id === newDoc.id ? { ...d, progress: i } : d))
        );
      }
      ...
      const aiSummary = `This ${newDoc.type.toUpperCase()} document contains ${
        ...
        extractedData.financials?.revenue
          ? `Revenue: ${extractedData.financials.revenue}.`
          : ""
      }`;
      ...
      <div className="flex items-start gap-2 p-3 bg-cyan-500/10 rounded-xl border border-cyan-500/20">
        ...
        <div className="text-xs text-cyan-200">
          ...
          <p className="text-cyan-300/70">
            ...
            I'll extract key information and pre-fill relevant sections, saving
            you time and ensuring consistency.
          </p>
        </div>
      </div>
    </div>
