...
function calculateMoodScore(inputs: WellnessInputs): number {
  ...
  const avgMood =
    inputs.moodScores.reduce((a, b) => a + b, 0) / inputs.moodScores.length;
  ...
  const variance =
    inputs.moodScores.length > 1
      ? inputs.moodScores.reduce(
          (sum, score) => sum + Math.pow(score - avgMood, 2),
          0
        ) / inputs.moodScores.length
      : 0;
...
function calculateProductivityScore(inputs: WellnessInputs): number {
  ...
  const completionRate =
    inputs.tasksPlanned > 0 ? inputs.tasksCompleted / inputs.tasksPlanned : 0.5;

  // Focus time score (optimal is 2-5 hours)
  let focusScore = 50;
  if (
    inputs.focusMinutes >= OPTIMAL.focusMinutes.min &&
    inputs.focusMinutes <= OPTIMAL.focusMinutes.max
  ) {
    focusScore = 100;
...
function calculateBalanceScore(inputs: WellnessInputs): number {
  ...
  if (inputs.sleepHours !== undefined) {
    if (
      inputs.sleepHours >= OPTIMAL.sleepHours.min &&
      inputs.sleepHours <= OPTIMAL.sleepHours.max
    ) {
      score += 10;
  ...
  if (
    inputs.exerciseMinutes !== undefined &&
    inputs.exerciseMinutes >= OPTIMAL.exerciseMinutes.min
  ) {
    score += 5;
  }
...
function calculateMomentumScore(inputs: WellnessInputs): number {
  ...
  const currentEstimate =
    (calculateMoodScore(inputs) + calculateProductivityScore(inputs)) / 2;
...
function generateInsights(
  ...
  breakdown: WellnessOutput["breakdown"]
): string[] {
  const insights: string[] = [];

  if (breakdown.mood >= 80) {
    insights.push(
      "Your mood has been excellent today! Keep up the positive energy."
    );
  } else if (breakdown.mood <= 40) {
    insights.push(
      "Your mood seems lower than usual. Consider taking a short break."
    );
  }

  if (
    inputs.tasksPlanned > 0 &&
    inputs.tasksCompleted / inputs.tasksPlanned >= 0.8
  ) {
    insights.push(
      `Great productivity! You completed ${inputs.tasksCompleted} of ${inputs.tasksPlanned} tasks.`
    );
  }

  if (inputs.focusMinutes >= 180) {
    insights.push(
      `You achieved ${Math.round(inputs.focusMinutes / 60)} hours of deep focus time.`
    );
  }

  if (inputs.backToBackMeetings >= 3) {
    insights.push(
      "You had several back-to-back meetings. Consider scheduling buffer time."
    );
  }

  if (inputs.streakDays >= 7) {
    insights.push(
      `Amazing! You're on a ${inputs.streakDays}-day streak of using The Brain.`
    );
  }
...
function generateRecommendations(
  ...
  if (breakdown.balance < 60) {
    recommendations.push(
      "Try to protect some focus time tomorrow by blocking your calendar."
    );
  }

  if (breakdown.mood < 60 && inputs.moodTrend === "declining") {
    recommendations.push(
      "Your mood has been declining. Consider scheduling something enjoyable."
    );
  }

  if (inputs.focusMinutes < 60) {
    recommendations.push(
      "Aim for at least 2 hours of uninterrupted focus time tomorrow."
    );
  }

  if (inputs.calendarDensity > 0.8) {
    recommendations.push(
      "Your calendar is very full. Consider declining or rescheduling some meetings."
    );
  }

  if (inputs.sleepHours !== undefined && inputs.sleepHours < 6) {
    recommendations.push(
      "Prioritize sleep tonight. Aim for 7-8 hours for optimal performance."
    );
  }

  if (breakdown.productivity < 60 && inputs.tasksPlanned > 5) {
    recommendations.push(
      "Consider planning fewer tasks tomorrow to increase completion rate."
    );
  }
