...
const initializeSampleData = () => {
  ...
  sampleExperts.forEach(expert => {
    const status =
      expert.avgRating < 6
        ...
        : expert.avgRating >= 8
          ...
          : "active";
    const trend =
      expert.avgRating >= 8 ? "up" : expert.avgRating < 6 ? "down" : "stable";
    ...
    expertPerformanceData.set(expert.id, {
      ...
      recentRatings: [
        expert.avgRating - 0.2,
        expert.avgRating,
        expert.avgRating + 0.1,
        expert.avgRating - 0.1,
        expert.avgRating + 0.2,
      ],
      trend,
      ...
      feedbackSummary: {
        commonImprovements:
          expert.avgRating < 7 ? ["more_specific", "better_sources"] : [],
        positiveThemes:
          expert.avgRating >= 7
            ? ["thorough analysis", "clear communication"]
            : [],
        negativeThemes:
          expert.avgRating < 6 ? ["too generic", "slow responses"] : [],
      },
      evolutionHistory: [],
    });
  });
...
export const expertEvolutionService = {
  ...
  runOvernightProcessing: async (): Promise<{
    ...
    allExperts.forEach(expert => {
      ...
      if (expert.averageRating < 4 && expert.status !== "suspended") {
        ...
        actionsRecommended.push({
          ...
          changes: {
            promptAdjustments: expert.feedbackSummary.commonImprovements.map(
              imp => `Address: ${imp}`
            ),
          },
        });
    ...
    const summary =
      `Overnight processing complete. Reviewed ${allExperts.length} experts. ` +
  ...
  generateMorningBrief: async (): Promise<{
    ...
    const needsAttention = allExperts
      .filter(
        e => e.status === "needs_improvement" || e.status === "under_review"
      )
    ...
    return {
      ...
      actionsApplied: allExperts.reduce(
        (sum, e) => sum + e.evolutionHistory.length,
        0
      ),
      topPerformers,
      needsAttention,
