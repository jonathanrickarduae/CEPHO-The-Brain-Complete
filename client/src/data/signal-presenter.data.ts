...
export function generateSampleBriefScript(): DailyBriefScript {
  return {
    ...
    scoreUpdate: {
      ...
      trend: "up",
      insight:
        "You're trending upward this week. Yesterday's focus on deep work paid off.",
    },
    priorities: [
      {
    ...
    focusRecommendation:
      "Block your morning for the Project A prep. Your afternoon is meeting-heavy, so front-load the deep work.",
    closing:
      "That's your brief. Chief of Staff is ready when you are. Have a productive day.",
  };
}
...
export function scriptDailyBrief(brief: DailyBriefScript): string {
  ...
  // Score update
  const trendWord =
    brief.scoreUpdate.trend === "up"
      ? "up"
      : brief.scoreUpdate.trend === "down"
        ? "down"
        : "holding steady";
  lines.push(
    `Yesterday you logged a ${brief.scoreUpdate.yesterdayScore}. You're trending ${trendWord}. ${brief.scoreUpdate.insight}`
  );
  lines.push("");
