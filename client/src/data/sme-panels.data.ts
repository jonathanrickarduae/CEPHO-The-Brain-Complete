...
export function getExpertsByPanelType(panelType: PanelType): AIExpert[] {
  if (panelType === "red_team") {
    ...
    const redTeamExperts = allExperts.filter(
      expert =>
...
export function getExpertPanelTypes(expert: AIExpert): PanelType[] {
  ...
  if (
    ["Left Field", "Celebrity Crossover"].includes(expert.category) &&
    !panels.includes("left_field")
  ) {
...
export function assemblePanelForTopic(
...
export function assembleThreePanelTeam(
  topic: string,
  teamSize: number = 3
): ThreePanelTeam {
...
export function getPanelStats(): Record<
  PanelType,
...
> {
  ...
  return {
    blue_team: {
      ...
      avgScore:
        stats.blue_team.count > 0
          ? Math.round(stats.blue_team.totalScore / stats.blue_team.count)
          : 0,
    },
    left_field: {
      count: stats.left_field.count,
      avgScore:
        stats.left_field.count > 0
          ? Math.round(stats.left_field.totalScore / stats.left_field.count)
          : 0,
    },
    red_team: {
      count: stats.red_team.count,
      avgScore:
        stats.red_team.count > 0
          ? Math.round(stats.red_team.totalScore / stats.red_team.count)
          : 0,
    },
  };
}
...
export function getTopPerformersByPanel(
  panelType: PanelType,
  limit: number = 5
): AIExpert[] {
...
export function getRecentlyUsedByPanel(
  panelType: PanelType,
  limit: number = 5
): AIExpert[] {
  return getExpertsByPanelType(panelType)
...
export const phaseRecommendedPanels: Record<
  number,
  { primary: PanelType; secondary: PanelType[] }
> = {
...
export function getRecommendedPanelsForPhase(phase: number): {
  primary: PanelType;
  secondary: PanelType[];
} {
  return (
    phaseRecommendedPanels[phase] || { primary: "blue_team", secondary: [] }
  );
}
