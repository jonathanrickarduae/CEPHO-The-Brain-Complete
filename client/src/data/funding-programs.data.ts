...
export const FUNDING_PROGRAMS: FundingProgram[] = [
  // UAE Programs
  {
    ...
    description:
      "Non-financial support program providing mentorship, services, and access to global experts without equity dilution.",
  ...
  },
  {
    ...
    provider: "Mohammed Bin Rashid Innovation Fund",
    description:
      "Government-backed credit guarantee providing access to financing without equity dilution.",
  ...
  },
  {
    ...
    description:
      "Microfinance loans for new entrepreneurs and SMEs in the UAE.",
  ...
  },
  {
    ...
    description:
      "Startup ecosystem support including funding, office space, and access to investors.",
  ...
  // UK Programs
  {
    ...
    provider: "HMRC",
    description:
      "Corporation Tax relief for qualifying R&D expenditure. Merged scheme for accounting periods from April 2024.",
  ...
  },
  {
    ...
    description:
      "Competitive funding for game-changing and disruptive innovation projects.",
  ...
  },
  {
    ...
    description:
      "Government-backed personal loans for starting or growing a business.",
  ...
  },
  {
    ...
    provider: "HMRC",
    description:
      "Tax relief scheme encouraging investment in early-stage companies.",
  ...
  },
  {
    ...
    provider: "HMRC",
    description:
      "Higher tax relief scheme for very early-stage seed investments.",
    ...
    website:
      "https://www.gov.uk/guidance/venture-capital-schemes-apply-for-the-seed-enterprise-investment-scheme",
  },
  {
    id: "uk-patent-box",
    name: "Patent Box",
    ...
    provider: "HMRC",
    description:
      "Reduced Corporation Tax rate on profits from patented inventions.",
...
export function assessFundingEligibility(
  ...
  if (eligibility.sectors && eligibility.sectors.length > 0) {
    ...
    if (
      ...
      missingCriteria.push(
        `Must be in priority sectors: ${eligibility.sectors.join(", ")}`
      );
    }
  }
  ...
  if (eligibility.excludedSectors && ideaData.sector) {
    maxScore += 10;
    if (
      !eligibility.excludedSectors.some(s =>
        ideaData.sector?.toLowerCase().includes(s.toLowerCase())
      )
    ) {
      score += 10;
  ...
  if (eligibility.maxTradingMonths !== undefined) {
    maxScore += 15;
    if (
      ideaData.tradingMonths !== undefined &&
      ideaData.tradingMonths <= eligibility.maxTradingMonths
    ) {
      score += 15;
      reasons.push(
        `Trading duration eligible (${ideaData.tradingMonths} months)`
      );
    } else if (ideaData.tradingMonths === undefined) {
      score += 7; // Partial score if unknown
      missingCriteria.push(
        `Must be trading less than ${eligibility.maxTradingMonths} months`
      );
    } else {
      missingCriteria.push(
        `Trading too long (max ${eligibility.maxTradingMonths} months)`
      );
    }
  }
  ...
  if (eligibility.maxEmployees !== undefined) {
    maxScore += 10;
    if (
      ideaData.employeeCount !== undefined &&
      ideaData.employeeCount <= eligibility.maxEmployees
    ) {
      score += 10;
      ...
      missingCriteria.push(
        `Too many employees (max ${eligibility.maxEmployees})`
      );
    }
  }
  ...
  if (eligibility.minAge !== undefined) {
    maxScore += 5;
    if (
      ideaData.founderAge !== undefined &&
      ideaData.founderAge >= eligibility.minAge
    ) {
      score += 5;
      ...
      missingCriteria.push(
        `Founder must be at least ${eligibility.minAge} years old`
      );
    }
  }
...
export function getProgramsByType(
