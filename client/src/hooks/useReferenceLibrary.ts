...
export function useReferenceLibrary({
  ...
  const updateReferenceStatus = useCallback(
    (referenceId: string, status: VerificationStatus, verifiedBy?: string) => {
      setReferences(prev =>
        prev.map(ref =>
          ref.id === referenceId
            ? {
  ...
  const addInsight = useCallback(
    (data: Omit<Insight, "id" | "createdAt" | "challenges" | "citations">) => {
  ...
  const removeCitationFromInsight = useCallback(
    (insightId: string, citationId: string) => {
      setInsights(prev =>
        prev.map(insight =>
          insight.id === insightId
            ? {
                ...insight,
                citations: insight.citations.filter(c => c.id !== citationId),
              }
            : insight
        )
      );
  ...
  const updateInsightStatus = useCallback(
    (insightId: string, status: VerificationStatus) => {
      setInsights(prev =>
        prev.map(insight =>
          insight.id === insightId
  ...
  const stats = useMemo(
    () => ({
      ...
      verifiedInsights: insights.filter(
        i => i.verificationStatus === "verified"
      ).length,
      pendingInsights: insights.filter(i => i.verificationStatus === "pending")
        .length,
      challengedInsights: insights.filter(
        i => i.verificationStatus === "challenged"
      ).length,
      ...
      totalChallenges: insights.reduce(
        (sum, i) => sum + i.challenges.length,
        0
      ),
    }),
    [insights, references]
  ...
  const generateChallenge = useCallback(
    (insight: Insight): Challenge => {
      ...
      if (validation.issues.some(i => i.type === "missing_source")) {
        question =
          "What is your primary source for this claim? Please provide a specific reference.";
      } else if (
        validation.issues.some(i => i.type === "potential_hallucination")
      ) {
        question =
          "Are you certain this is accurate, or could you be confabulating? Please verify against current data.";
      } else if (validation.issues.some(i => i.type === "opinion_as_fact")) {
        question =
          "Is this a verifiable fact or your professional opinion? Please clarify.";
      } else if (insight.confidence === "speculative") {
        question =
          "What assumptions underlie this speculation? What would change your assessment?";
      }
      ...
      try {
        ...
        if (validation.requiredReferences.length > 0) {
          recommendations.push(
            `Add references: ${validation.requiredReferences.join(", ")}`
          );
        }
