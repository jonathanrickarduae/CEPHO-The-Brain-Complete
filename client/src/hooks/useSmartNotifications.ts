...
export function useSmartNotifications() {
  const [notifications, setNotifications] =
    useState<SmartNotification[]>(loadNotifications);
  ...
  const shouldShowNotification = useCallback(
    (type: NotificationType, priority: NotificationPriority): boolean => {
      ...
      if (patterns.lastNotificationTime) {
        const timeSinceLastNotification =
          Date.now() - patterns.lastNotificationTime.getTime();
    ...
    (context: {
      ...
      if (lastActivity) {
        ...
        if (hoursSinceActivity > 24) {
          addNotification({
            ...
            title: "We miss you!",
            message:
              "Your Chief of Staff has been waiting. Check in to maintain your streak.",
            priority: "medium",
            icon: "👋",
          });
          return;
      ...
      if (streakDays && streakDays > 0 && streakDays % 7 === 0) {
        addNotification({
          ...
          message:
            "Amazing consistency! Keep it up to unlock new achievements.",
          priority: "low",
          icon: "🏆",
        });
      }
  ...
  const generateInsight = useCallback(
    (insight: { title: string; message: string; actionUrl?: string }) => {
      addNotification({
        type: "insight",
        title: insight.title,
  ...
  const visibleNotifications = notifications.filter(
    n => !n.dismissed && (!n.expiresAt || n.expiresAt > new Date())
  );
