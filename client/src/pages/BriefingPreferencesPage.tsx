    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {ALL_SECTIONS.map(section => (
          ...
          >
            <CheckCircle
              className={`w-4 h-4 ${enabled.includes(section.key) ? "text-blue-500" : "text-gray-300"}`}
            />
            {section.label}
          </button>
        ))}
...
function BriefingCard({
  ...
  const submitFeedback =
    trpc.briefingPersonalisation.submitFeedback.useMutation();
          ...
          {content.strategic_focus != null && (
            <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
              ...
              <p className="text-sm text-blue-800">
                {String(content.strategic_focus)}
              </p>
            </div>
          )}
...
function AgentInsightsPanel() {
  const { data: insights, isLoading } =
    trpc.briefingPersonalisation.getAgentInsights.useQuery({ limit: 20 });
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-500">
            Agent insights will appear here after the 05:00 research run.
          </p>
        </div>
      ) : (
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium ${agentColors[insight.agentKey] ?? "bg-gray-100 text-gray-600"}`}
                    >
...
export default function BriefingPreferencesPage() {
  ...
  const { data: prefs, refetch: refetchPrefs } =
    trpc.briefingPersonalisation.getPreferences.useQuery();
  const { data: briefings, refetch: refetchBriefings } =
    trpc.briefingPersonalisation.listBriefings.useQuery({ limit: 10 });

  const updatePrefs =
    trpc.briefingPersonalisation.updatePreferences.useMutation({
      onSuccess: () => refetchPrefs(),
    });

  const generateBriefing =
    trpc.briefingPersonalisation.generateBriefing.useMutation({
      onSuccess: (data: {
        briefingId: number;
        title: string;
        content: unknown;
  ...
  const enabledSections =
    (prefs?.enabledSections as string[] | null) ?? ALL_SECTIONS.map(s => s.key);

  return (
    <PageShell
      title="Briefing Preferences"
      subtitle="Personalise Victoria's morning briefing and review agent insights"
    >
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="space-y-5">
              <ToggleOption
                ...
                value={
                  (prefs?.preferredLength ?? "standard") as
                    | "brief"
                    | "standard"
                    | "detailed"
                }
              ...
              <SectionToggle
                ...
                onChange={sections =>
                  handleUpdate({ enabledSections: sections })
                }
              />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                  ...
                  <button
                    onClick={() =>
                      handleUpdate({ includeWeather: !prefs?.includeWeather })
                    }
                    className={`w-10 h-5 rounded-full transition-colors ${prefs?.includeWeather ? "bg-blue-500" : "bg-gray-300"}`}
                  >
                    <span
                      className={`block w-4 h-4 rounded-full bg-white shadow transition-transform mx-0.5 ${prefs?.includeWeather ? "translate-x-5" : "translate-x-0"}`}
                    />
                  </button>
                </div>
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                  ...
                  <button
                    onClick={() =>
                      handleUpdate({
                        includeMarketData: !prefs?.includeMarketData,
                      })
                    }
                    className={`w-10 h-5 rounded-full transition-colors ${prefs?.includeMarketData ? "bg-blue-500" : "bg-gray-300"}`}
                  >
                    <span
                      className={`block w-4 h-4 rounded-full bg-white shadow transition-transform mx-0.5 ${prefs?.includeMarketData ? "translate-x-5" : "translate-x-0"}`}
                    />
                  </button>
                </div>
              </div>
