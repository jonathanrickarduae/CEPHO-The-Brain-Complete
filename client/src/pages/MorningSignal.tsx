...
export default function MorningSignal() {
  ...
  const generateSignalItems = (): SignalItem[] => {
    ...
    if (eveningReviewData) {
      ...
      if (metadata?.overnightWork) {
        (metadata.overnightWork as any[]).forEach(
  ...
  const generateVoiceMutation =
    trpc.expertEvolution.generateVoice.useMutation();
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      toast.success(
        'Morning Signal opened for printing. Use "Save as PDF" in the print dialog.'
      );
    } catch (error) {
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="p-4 rounded-xl border bg-card animate-pulse"
            >
              ...
              {acceptedItems.length > 0 ? (
                acceptedItems.map(item => (
                  <div
                    ...
                    onClick={() =>
                      item.actionUrl && setLocation(item.actionUrl)
                    }
                  >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-3">
                          <Button
                            ...
                            onClick={() => {
                              toast.success(
                                `"${item.title}" marked as ready to start`
                              );
                            }}
                          >
