...
import {
  Activity, TrendingUp, Award, Clock, CheckCircle,
  XCircle, Filter, Users, Brain, RefreshCw,
} from "lucide-react";
...
export default function AIAgentsMonitoringPage() {
  ...
  const {
    data: agentsData, isLoading, error, refetch,
  ...
  const filteredAgents = agents.filter(agent => {
    const categoryMatch = selectedCategory === "All" || agent.specialization.includes(selectedCategory);
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "learning": return "bg-primary/20 text-primary";
      case "idle":     return "bg-gray-500/20 text-muted-foreground";
      ...
      {!isLoading && !error && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { icon: Users,     color: "text-primary",    label: "Total Agents",    value: agentsData?.totalAgents || 0 },
              { icon: Activity,  color: "text-green-400",  label: "Active Now",      value: agentsData?.activeAgents || 0 },
              { icon: Award,     color: "text-yellow-400", label: "Avg Rating",      value: agents.length > 0 ? (agents.reduce((s,a) => s + a.performanceRating, 0) / agents.length).toFixed(1) : "0" },
              { icon: TrendingUp,color: "text-purple-400", label: "Tasks Completed", value: agents.reduce((s,a) => s + a.tasksCompleted, 0) },
            ].map(s => (
          ...
          <div className="bg-card rounded-xl p-4 border border-border flex flex-wrap gap-4 items-end">
            ...
            <div>
              ...
              >
                {AGENT_CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}
              </select>
            </div>
            <div>
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded-full ${getStatusColor(agent.status)}`}>
                      {agent.status}
                    </span>
                  </div>
                  <div className="flex items-center gap-1 text-yellow-400 shrink-0 ml-2">
                    <Award className="w-3 h-3" />
                    <span className="text-sm font-semibold">{agent.performanceRating}</span>
                  </div>
                </div>
          ...
          {selectedAgentData && (
            <div className="bg-card rounded-xl border border-primary/30 p-5">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h2 className="text-base font-bold">{selectedAgentData.name}</h2>
                  <p className="text-sm text-muted-foreground">{selectedAgentData.specialization}</p>
                  {selectedAgentReport && (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                {[
                  { label: "Rating",       value: `${selectedAgentData.performanceRating}/100`, color: "text-yellow-400" },
                  { label: "Tasks Done",   value: selectedAgentData.tasksCompleted,             color: "text-foreground" },
                  { label: "Success Rate", value: `${selectedAgentData.successRate}%`,          color: "text-green-400" },
                  { label: "Response",     value: `${selectedAgentData.avgResponseTime.toFixed(2)}s`, color: "text-blue-400" },
                ].map(m => (
              ...
              {selectedAgentReport && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    { title: "Improvements",    items: selectedAgentReport.improvements,    color: "text-green-400",  icon: TrendingUp },
                    { title: "New Learnings",   items: selectedAgentReport.newLearnings,    color: "text-primary",    icon: Brain },
                    { title: "Suggestions",     items: selectedAgentReport.suggestions,     color: "text-yellow-400", icon: Award },
                    { title: "Research Topics", items: selectedAgentReport.researchTopics,  color: "text-purple-400", icon: Activity },
                  ].map(section => (
              ...
              {selectedAgentReport?.requestsForApproval?.length > 0 && (
                <div className="mt-4">
                  <h3 className="text-sm font-semibold mb-3">Requests for Approval</h3>
                  <div className="space-y-3">
                    {selectedAgentReport.requestsForApproval.map(request => (
                      <div key={request.id} className="bg-muted/40 rounded-lg p-3">
                        <div className="flex items-start justify-between gap-3 mb-2">
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium">{request.type}</p>
                            <p className="text-xs text-muted-foreground mt-0.5">{request.description}</p>
                            <p className="text-xs text-muted-foreground/70 mt-1">{request.reasoning}</p>
                            <p className="text-xs text-primary mt-1">Impact: {request.estimatedImpact}</p>
                          </div>
