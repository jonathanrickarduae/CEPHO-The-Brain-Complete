...
import {
  Bot, RefreshCw, Brain, CheckCircle2, Clock, Cpu, FileText,
  Lightbulb, Play, Search, Shield, Star, TrendingUp, Users, Zap,
  AlertCircle, ThumbsUp, ThumbsDown, Activity, Award, BookOpen,
  ChevronRight
} from "lucide-react";
...
export default function AIAgentsPage() {
  ...
  const { data: agentsData, isLoading: agentsLoading, error: agentsError, refetch } =
  ...
  const stats = agentsData
    ? {
        ...
        avgPerformance: agents.length > 0
          ? agents.reduce((s, a) => s + a.performanceRating, 0) / agents.length : 0,
  ...
  const { data: approvalQueue, isLoading: loadingApprovals, refetch: refetchApprovals } =
  ...
  const { data: learningFeed } =
    trpc.agentEngine.getLearningFeed.useQuery();
  ...
  // ── Helpers ───────────────────────────────────────────────────────────────
  const engineAgents = knowledgeMap?.agents.filter(agent => {
  ...
  const getStatusColor = (s: string) =>
    s === "active" ? "bg-green-500" : s === "learning" ? "bg-primary" : s === "idle" ? "bg-yellow-500" : "bg-gray-500";
  const getPerfColor = (r: number) =>
    r >= 90 ? "text-green-400" : r >= 75 ? "text-blue-400" : r >= 60 ? "text-yellow-400" : "text-red-400";
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          {(approvalQueue?.pendingCount ?? 0) > 0 && (
            <Button size="sm" variant="outline" onClick={() => setActiveTab("approvals")} className="gap-1.5">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          {!agentsLoading && !agentsError && (
            <div className="space-y-5">
              ...
              {stats && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    ...
                    { label: "Tasks Done",      value: stats.totalTasksCompleted,                color: "text-foreground" },
                  ].map(s => (
                    ...
                    <div className="bg-card rounded-xl p-4 border border-border hover:border-primary/40 hover:shadow-md transition-all cursor-pointer">
                      ...
                      <div className="mb-3">
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className={`text-xs font-bold ${getPerfColor(agent.performanceRating)}`}>
                            {agent.performanceRating}/100
                          </span>
                        </div>
                        <div className="w-full bg-muted rounded-full h-1.5">
                          <div className="bg-primary h-1.5 rounded-full" style={{ width: `${agent.performanceRating}%` }} />
                        </div>
                      </div>
              ...
              {filteredAgents.length === 0 && (
                <div className="text-center py-8 text-muted-foreground text-sm">No agents found in this category</div>
              )}
            </div>
          )}
        </TabsContent>
              ...
              {knowledgeMap?.categories.map(cat => (
                ...
                >
                  <CardContent className="p-4">
                    <div className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium border mb-2 ${CATEGORY_COLORS[cat.name] ?? "bg-muted text-muted-foreground"}`}>
                      {cat.name.charAt(0).toUpperCase() + cat.name.slice(1)}
                    </div>
                    <div className="text-2xl font-bold text-foreground">{cat.count}</div>
                    <div className="text-xs text-muted-foreground">agents</div>
                    <div className="mt-2 text-xs text-muted-foreground line-clamp-2">
                      {cat.agents.slice(0, 2).join(", ")}{cat.agents.length > 2 ? ` +${cat.agents.length - 2}` : ""}
                    </div>
                  </CardContent>
                </Card>
              ))}
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <div className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs mt-1 border ${CATEGORY_COLORS[agent.category] ?? "bg-muted"}`}>
                          {agent.category}
                        </div>
                      </div>
                      <Button
                        ...
                        onClick={e => { e.stopPropagation(); setSelectedAgent(agent.id); setActiveTab("execute"); }}
                      >
                    ...
                    <div className="space-y-1">
                      ...
                      {agent.capabilities.length > 3 && (
                        <div className="text-xs text-muted-foreground ml-4">+{agent.capabilities.length - 3} more</div>
                      )}
                    </div>
                    {selectedAgent === agent.id && (
                      <div className="mt-3 pt-3 border-t border-border/30">
                        <div className="text-xs font-medium text-foreground mb-1.5">Daily Tasks:</div>
                        {agent.dailyTasks.map(task => (
                          <div key={task} className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1">
                        ...
                        <Button
                          ...
                          onClick={e => { e.stopPropagation(); generateReport.mutate({ agentId: agent.id }); setActiveTab("reports"); }}
                        >
            ...
            <Card className="bg-card border-border/50">
              ...
              <CardContent className="space-y-2">
                {learningFeed.learningItems.map(item => (
                  <div key={item.agentId} className="flex items-center justify-between p-3 rounded-lg bg-muted/20 border border-border/30">
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-2 rounded-full animate-pulse ${item.status === "researching" ? "bg-blue-400" : "bg-green-400"}`} />
                      <div>
                        <div className="text-sm font-medium text-foreground">{item.agentName}</div>
                        <div className="text-xs text-muted-foreground">{item.researchTopic}</div>
                      </div>
                    </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="bg-card border-border/50">
              ...
              <CardContent className="space-y-3">
                <div>
                  <label className="text-xs text-muted-foreground mb-1.5 block">Select Agent</label>
                  <select
                    value={selectedAgent ?? ""}
                  ...
                  >
                    ...
                    {knowledgeMap?.categories.map(cat => (
                      <optgroup key={cat.name} label={cat.name.charAt(0).toUpperCase() + cat.name.slice(1)}>
                        {cat.agents.map(agentName => {
                          const a = knowledgeMap.agents.find(x => x.name === agentName);
                ...
                {selectedAgentData && (
                  <div className={`p-2 rounded-lg text-xs border ${CATEGORY_COLORS[selectedAgentData.category] ?? "bg-muted"}`}>
                    <div className="font-medium mb-1">{selectedAgentData.name}</div>
                    <div className="opacity-80">Capabilities: {selectedAgentData.capabilities.slice(0, 3).join(", ")}</div>
                  </div>
                )}
                <div>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (selectedAgent && taskInput.trim()) {
                      executeTask.mutate({ agentId: selectedAgent, taskDescription: taskInput, context: taskContext || undefined });
                    }
                  }}
                  disabled={!selectedAgent || !taskInput.trim() || executeTask.isPending}
                >
                  {executeTask.isPending ? (
                    <><Cpu className="w-4 h-4 mr-2 animate-spin" />Agent Working...</>
                  ) : (
              ...
              <CardContent>
                {executeTask.isPending ? (
                  ...
                  <div className="space-y-3">
                    <div className="p-3 rounded-lg bg-green-500/5 border border-green-500/20">
                      <div className="flex items-center gap-2 mb-2">
                        <CheckCircle2 className="w-4 h-4 text-green-400" />
                        <span className="text-xs font-medium text-green-400">Task Completed</span>
                      </div>
                      <div className="text-sm text-foreground whitespace-pre-wrap leading-relaxed">{taskResult}</div>
                    </div>
                    <Button variant="outline" size="sm" className="w-full"
                      onClick={() => { setTaskResult(null); setTaskInput(""); setTaskContext(""); }}>
                  ...
                  <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
                    ...
                    <p className="text-sm">Select an agent and describe a task to get started.</p>
                  </div>
                )}
                {executeTask.isError && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="bg-card border-border/50">
              ...
              <CardContent className="space-y-3">
                ...
                <Button
                  className="w-full"
                  onClick={() => selectedAgent && generateReport.mutate({ agentId: selectedAgent })}
                  disabled={!selectedAgent || generateReport.isPending}
                >
                  {generateReport.isPending ? (
                    <><Cpu className="w-4 h-4 mr-2 animate-spin" />Generating...</>
                  ) : (
                    <><FileText className="w-4 h-4 mr-2" />Generate Today's Report</>
                  )}
                </Button>
              </CardContent>
            </Card>
                ...
                {generateReport.isPending ? (
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-foreground">{reportResult.agentName as string}</span>
                      <Badge variant="outline" className="text-xs">{reportResult.date as string}</Badge>
                    </div>
                    {Array.isArray(reportResult.tasksCompleted) && (
                      <div>
                        <div className="text-xs font-medium text-muted-foreground mb-1">Tasks Completed</div>
                        {(reportResult.tasksCompleted as string[]).map((t, i) => (
                          <div key={i} className="flex items-center gap-1.5 text-xs text-foreground mb-1">
                            <CheckCircle2 className="w-3 h-3 text-green-400 flex-shrink-0" />{t}
                          </div>
                        ))}
                      </div>
                    )}
                    {reportResult.achievements && (
                      <div>
                        ...
                        <p className="text-xs text-foreground">{reportResult.achievements as string}</p>
                      </div>
                    )}
                    {Array.isArray(reportResult.suggestions) && (
                      <div>
                        ...
                        {(reportResult.suggestions as string[]).map((s, i) => (
                          <div key={i} className="flex items-center gap-1.5 text-xs text-foreground mb-1">
                            <Lightbulb className="w-3 h-3 text-amber-400 flex-shrink-0" />{s}
                          </div>
                        ))}
                      </div>
                    )}
                    {(reportResult.capabilityRequest as any)?.title && (
                      <div className="p-2 rounded-lg bg-amber-500/10 border border-amber-500/20">
                        <div className="text-xs font-medium text-amber-400 mb-1">Capability Request (Pending CoS Approval)</div>
                        <div className="text-xs text-foreground">{(reportResult.capabilityRequest as any).title}</div>
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm">Select an agent to generate their daily report.</p>
                  </div>
                )}
              </CardContent>
            </Card>
        ...
        <TabsContent value="approvals" className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {[
              { label: "Pending",         value: approvalQueue?.pendingCount ?? 0,          icon: <Shield className="w-4 h-4" />,    color: "text-amber-400" },
              { label: "Approved Today",  value: approvalQueue?.approvedToday ?? 0,          icon: <ThumbsUp className="w-4 h-4" />,  color: "text-green-400" },
              { label: "Recent Decisions",value: approvalQueue?.recentHistory?.length ?? 0,  icon: <Activity className="w-4 h-4" />, color: "text-blue-400" },
            ].map(s => (
          ...
          {loadingApprovals ? (
            ...
            <div className="space-y-3">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-medium text-foreground text-sm">{req.agentName}</span>
                          <Badge variant="outline" className={`text-xs ${CATEGORY_COLORS[req.category] ?? ""}`}>{req.category}</Badge>
                          <Badge variant="outline" className={`text-xs ${req.priority === "high" ? "text-red-400 border-red-500/30" : req.priority === "medium" ? "text-amber-400 border-amber-500/30" : "text-muted-foreground"}`}>
                            {req.priority}
                          </Badge>
                        </div>
                        <div className="text-sm font-medium text-foreground mb-1">{req.title}</div>
                        <div className="text-xs text-muted-foreground mb-2">{req.description}</div>
                        <div className="text-xs text-green-400">
                          <TrendingUp className="w-3 h-3 inline mr-1" />{req.estimatedImpact}
                        </div>
                      </div>
                      <div className="flex flex-col gap-2 flex-shrink-0">
                        <Button
                          ...
                          onClick={() => processApproval.mutate({ requestId: req.id, agentId: req.agentId, decision: "approved" })}
                        ...
                        <Button
                          ...
                          onClick={() => processApproval.mutate({ requestId: req.id, agentId: req.agentId, decision: "denied" })}
                          disabled={processApproval.isPending}
                        >
              ...
              {(approvalQueue?.recentHistory.length ?? 0) > 0 && (
                <div>
                  <div className="text-xs font-medium text-muted-foreground mb-2 mt-4">Recent Decisions</div>
                  {approvalQueue?.recentHistory.map((h, i) => (
                    <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-muted/20 border border-border/30 mb-2">
                      <div className="text-sm text-foreground">{h.notes}</div>
                      <Badge variant="outline" className={`text-xs ${h.decision === "approved" ? "text-green-400 border-green-500/30" : "text-red-400 border-red-500/30"}`}>
                        {h.decision}
                      </Badge>
                    </div>
                  ))}
            ...
            <>
              {leaderboard?.topPerformer && (
                <Card className="bg-gradient-to-r from-amber-500/10 to-yellow-500/10 border-amber-500/30">
                  <CardContent className="p-4 flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-xs text-amber-400 font-medium mb-0.5">Top Performer</div>
                      <div className="font-semibold text-foreground">{leaderboard.topPerformer.name}</div>
                      <div className="text-xs text-muted-foreground">{leaderboard.topPerformer.specialization}</div>
                    </div>
                    <div className="ml-auto text-right">
                      <div className="text-2xl font-bold text-amber-400">{leaderboard.topPerformer.rating.toFixed(1)}</div>
              ...
              <div className="space-y-2">
                {leaderboard?.leaderboard.map((agent, index) => (
                  <div key={agent.id} className="flex items-center gap-3 p-3 rounded-lg bg-muted/20 border border-border/30">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${index === 0 ? "bg-amber-500 text-white" : index === 1 ? "bg-slate-400 text-white" : index === 2 ? "bg-amber-700 text-white" : "bg-muted text-muted-foreground"}`}>
                      {index + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium text-foreground truncate">{agent.name}</span>
                        <Badge variant="outline" className={`text-xs flex-shrink-0 ${CATEGORY_COLORS[agent.category] ?? ""}`}>{agent.category}</Badge>
                      </div>
                      <div className="flex items-center gap-3 mt-1">
                        <Progress value={agent.successRate} className="h-1 flex-1 max-w-[120px]" />
                        <span className="text-xs text-muted-foreground">{agent.successRate}% success</span>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div className="text-sm font-bold text-foreground">{agent.rating.toFixed(1)}</div>
