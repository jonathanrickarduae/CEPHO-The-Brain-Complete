...
const generateMockScores = (): Record<
  string,
  { score: number; trend: number; lastUpdated: string }
> => {
  const scores: Record<
    string,
    { score: number; trend: number; lastUpdated: string }
...
export default function KpiDashboard() {
  ...
  const [expandedDomains, setExpandedDomains] = useState<Set<string>>(
    new Set(KPI_DOMAINS.map(d => d.domain))
  );
  ...
  const domainAverages = useMemo(() => {
    ...
    KPI_DOMAINS.forEach(d => {
      ...
      const total = categories.reduce(
        (sum, cat) => sum + (mockScores[cat.id]?.score || 0),
        0
      );
  ...
  const overallScore = useMemo(() => {
    const total = Object.values(mockScores).reduce(
      (sum, s) => sum + s.score,
      0
    );
  ...
  const weakSpots = useMemo(() => {
    return KPI_CATEGORIES.filter(
      cat => (mockScores[cat.id]?.score || 0) < 50
    ).sort(
      (a, b) => (mockScores[a.id]?.score || 0) - (mockScores[b.id]?.score || 0)
    );
  }, []);
  ...
  const strongAreas = useMemo(() => {
    return KPI_CATEGORIES.filter(
      cat => (mockScores[cat.id]?.score || 0) >= 80
    ).sort(
      (a, b) => (mockScores[b.id]?.score || 0) - (mockScores[a.id]?.score || 0)
    );
  }, []);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-black text-white py-4 px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <Badge
            variant="outline"
            className="border-fuchsia-500 text-fuchsia-400"
          >
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select
                value={sortBy}
                onValueChange={v => setSortBy(v as "score" | "trend" | "name")}
              >
            ...
            >
              ...
              {selectedDomain === "all" ? (
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-4">
                              <div className="flex items-center gap-2">
                                ...
                                <span
                                  className={`text-lg font-bold ${avgScore >= 70 ? "text-emerald-500" : avgScore >= 50 ? "text-amber-500" : "text-red-500"}`}
                                >
                                  {avgScore}
                                </span>
                              </div>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>
                      {KPI_DOMAINS.find(d => d.domain === selectedDomain)?.name}{" "}
                      Categories
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
