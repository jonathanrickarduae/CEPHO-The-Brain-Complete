...
const EXPERT_PROFILES = {
  strategy: {
    ...
    role: "Strategy Lead",
    avatar: "AC",
    specialty: "Business Strategy & Market Analysis",
  },
  engineering: {
    name: "Sarah Kim",
    role: "Engineering Lead",
    avatar: "SK",
    specialty: "Technical Architecture & Development",
  },
  marketing: {
    name: "Marcus Johnson",
  ...
  finance: {
    ...
    role: "Finance Expert",
    avatar: "DP",
    specialty: "Financial Modeling & Funding",
  },
  design: {
    name: "Lisa Wang",
    role: "Design Lead",
    avatar: "LW",
    specialty: "UX/UI & Product Design",
  },
  operations: {
    name: "James Miller",
    role: "Operations Expert",
    avatar: "JM",
    specialty: "Process & Scaling",
  },
  research: {
    name: "Dr. Priya Sharma",
    role: "Research Lead",
    avatar: "PS",
    specialty: "Market Research & Data Analysis",
  },
};
...
const MOCK_PENDING_TASKS: PendingTask[] = [
  ...
  },
  {
    ...
    description:
      "Deep dive into Competitor X's new AI bid tool - verify 15% efficiency claims",
    category: "Research",
    source: "daily-brief",
    type: "team",
    status: "pending",
...
const MISSION_TEMPLATES = [
  {
    ...
    description: "Emails, presentations, or reports",
    icon: Mail,
  ...
  },
  {
    ...
    description: "Deep dive into a topic or market",
    icon: Search,
  ...
  },
  {
    ...
    description: "Define your own objective",
    icon: Lightbulb,
    type: "team" as TaskType,
  },
];
...
export default function AIExperts() {
    ...
    if (isRunning && timeRemaining > 0) {
      interval = setInterval(() => {
        ...
        if (activeTask) {
          setActiveTask(prev =>
            prev
              ? { ...prev, progress: Math.min(prev.progress + 0.5, 95) }
              : null
          );
        }
      }, 1000);
  ...
  const analyzeAndRecommendTeam = (task?: PendingTask) => {
    ...
    if (
      missionLower.includes("marketing") ||
      missionLower.includes("brand") ||
      missionLower.includes("social")
    ) {
      team.push("marketing");
    }
    if (
      missionLower.includes("build") ||
      missionLower.includes("develop") ||
      missionLower.includes("technical")
    ) {
      team.push("engineering");
      team.push("design");
    }
    if (
      missionLower.includes("budget") ||
      missionLower.includes("finance") ||
      missionLower.includes("investor")
    ) {
      team.push("finance");
    }
    if (
      missionLower.includes("research") ||
      missionLower.includes("analyze") ||
      missionLower.includes("competitor")
    ) {
      team.push("research");
    }
    if (
      missionLower.includes("legal") ||
      missionLower.includes("contract") ||
      missionLower.includes("compliance")
    ) {
      team.push("legal");
    }
    ...
    if (team.length < 3) {
      ...
      if (!team.includes("operations") && team.length < 3)
  ...
  const generateKickoffQuestions = () => {
    const questions: KickoffQuestion[] = [
      {
        id: 1,
        question:
          "Do you have specific requirements or constraints we should know about?",
        expertFrom: "strategy",
      },
      {
        id: 2,
        question: "Should we prioritize speed over comprehensiveness?",
        expertFrom: "strategy",
      },
      {
        id: 3,
        question:
          "Can we proceed with our recommendations if you're unavailable?",
        expertFrom: "strategy",
      },
    ];
    ...
    if (approvedTeam.includes("research")) {
      questions.push({
        id: 4,
        question: "Should we include competitor analysis in our research?",
        expertFrom: "research",
      });
    }
    if (approvedTeam.includes("finance")) {
      questions.push({
        id: 5,
        question: "Do you need detailed financial projections?",
        expertFrom: "finance",
      });
    }
  ...
  const startActiveWork = () => {
    ...
    const initialDialogue: DialogueMessage[] = [
      ...
      },
      {
        ...
        message:
          "Team is assembled and ready. We'll deliver the first draft within the timeline.",
        timestamp: new Date(),
      },
    ];
      ...
      const response: DialogueMessage = {
        ...
        name:
          activeTask.type === "twin"
            ? "Chief of Staff"
            : EXPERT_PROFILES[approvedTeam[0]]?.name || "Team",
        message:
          "Understood. I'll incorporate that feedback into the deliverable.",
        timestamp: new Date(),
      };
  ...
  const moveToReview = () => {
    ...
    const reviewDialogue: DialogueMessage = {
      ...
      name: "Chief of Staff",
      message:
        "The deliverable is ready for your review. I've conducted a quality check and have some recommendations.",
      timestamp: new Date(),
      needsResponse: true,
    };
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Your favorite experts for quick access
                  </p>
                </div>
                <button
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-6 py-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Multi-expert collaborative problem solving
                  </p>
                </div>
                <button
                  onClick={() => setViewMode("action")}
                ...
                <CardContent className="space-y-3">
                  {tasks.filter(t => t.status !== "completed").length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p>
                        No pending tasks. Create a new mission above or action
                        items from The Signal.
                      </p>
                    </div>
                  ) : (
                    tasks
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                >
                                  {task.type === "twin" ? (
                                    <>
                                      <User className="w-3 h-3 mr-1" /> Chief of
                                      Staff
                                    </>
                                  ) : (
                                    <>
                                      <Users className="w-3 h-3 mr-1" /> Expert
                                      Team
                                    </>
                                  )}
                                </Badge>
                                  ...
                                  onClick={() => {
                                    ...
                                    if (task.status === "active") {
                                      ...
                                      setTimeRemaining(
                                        Math.max(
                                          0,
                                          (deliverableTime || 30) * 60 -
                                            Math.floor(task.progress * 0.6 * 60)
                                        )
                                      );
                                    }
                                  }}
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-foreground">
                        {isRecording
                          ...
                          : "Tap to add a task by voice"}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Describe what you need done and I'll route it
                        appropriately
                      </p>
                    </div>
                  </div>
              ...
              <Card className="bg-card/60 border-border">
                ...
                <CardContent className="space-y-4">
                  <p className="text-muted-foreground mb-4">
                    Based on your mission, I recommend these experts. Tap to add
                    or remove.
                  </p>
                  ...
                  <div className="flex gap-3 pt-4">
                    ...
                    >
                      <ArrowRight className="w-4 h-4 mr-2" />
                      Start Kickoff (
                      {approvedTeam.length || recommendedTeam.length} experts)
                    </Button>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-card/60 border-border">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge>
                      {currentQuestionIndex + 1} / {kickoffQuestions.length}
                    </Badge>
                  </div>
                </CardHeader>
                  ...
                  <div className="p-6 rounded-xl bg-secondary/30 border border-border">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center font-bold text-sm text-primary">
                        {
                          EXPERT_PROFILES[
                            kickoffQuestions[currentQuestionIndex].expertFrom
                          ]?.avatar
                        }
                      </div>
                      <div>
          ...
          {phase === "timeline" && (
            <div className="max-w-2xl mx-auto space-y-6">
              <Card className="bg-card/60 border-border">
                <CardHeader>
                  <CardTitle className="text-foreground">
                    When do you need the first deliverable?
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
          ...
          {phase === "review" && activeTask && (
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div className="flex gap-2 mb-4">
                ...
                <Button
                  variant={
                    validationMode === "challenge" ? "default" : "outline"
                  }
                  size="sm"
                  ...
                  <Card className="bg-yellow-500/5 border-yellow-500/30">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="p-4 rounded-xl bg-purple-500/10 border border-purple-500/30">
                        ...
                        <ul className="space-y-2 text-sm text-muted-foreground">
                          <li className="flex items-start gap-2">
                            ...
                            <span>
                              Content aligns with your communication style
                            </span>
                          </li>
                          <li className="flex items-start gap-2">
                            <CheckCircle2 className="w-4 h-4 text-green-400 mt-0.5" />
                          ...
                          <li className="flex items-start gap-2">
                            ...
                            <span>
                              Consider adding more specific metrics (optional)
                            </span>
                          </li>
                        </ul>
                      </div>
                      ...
                      <div className="flex gap-3">
                        ...
                        >
                          <RefreshCw className="w-4 h-4 mr-2" /> Request
                          Revision
                        </Button>
                        <Button
                          className="flex-1 bg-green-500 hover:bg-green-600"
              ...
              {validationMode === "review" && (
                <>
                  <Card className="bg-amber-500/5 border-amber-500/30">
                    ...
                    <CardContent className="space-y-4">
                      <p className="text-sm text-muted-foreground">
                        Each statement is classified as Fact, Analysis, Opinion,
                        or Speculation. Facts require source citations. All
                      ...
                      <div className="space-y-3">
                        {[
                          {
                            content:
                              "Market size is projected to reach $50B by 2027",
                            classification: "fact" as StatementClassification,
                            confidence: "high" as ConfidenceLevel,
                            needsSource: true,
                          },
                          {
                            content:
                              "Based on the data, we recommend focusing on enterprise clients",
                            classification:
                              "analysis" as StatementClassification,
                            confidence: "medium" as ConfidenceLevel,
                            needsSource: false,
                          },
                          {
                            content:
                              "The competitor's approach seems less efficient",
                            classification:
                              "opinion" as StatementClassification,
                      ...
                      <div className="flex gap-3">
                        ...
                        >
                          <FileText className="w-4 h-4 mr-2" /> Request
                          Citations
                        </Button>
                        <Button
                          className="flex-1 bg-green-500 hover:bg-green-600"
              ...
              {validationMode === "challenge" && (
                <>
                  <Card className="bg-purple-500/5 border-purple-500/30">
                    ...
                    <CardContent className="space-y-4">
                      <p className="text-sm text-muted-foreground">
                        Your Chief of Staff will challenge each expert's
                        statements to verify accuracy and identify potential
                            ...
                            >
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-sm font-medium text-purple-400">
                                  Chief of Staff Challenge #{idx + 1}
                                </span>
                              </div>
                              ...
                              <div className="flex gap-2 mt-3">
                                ...
                                >
                                  <AlertCircle className="w-3 h-3 mr-1" /> Needs
                                  Revision
                                </Button>
                              </div>
                            </div>
                      ...
                      <div className="p-4 rounded-xl bg-secondary/30 border border-border">
                        ...
                        <p className="text-sm text-muted-foreground">
                          The expert must respond to each challenge with
                          evidence or clarification. Unsubstantiated claims will
