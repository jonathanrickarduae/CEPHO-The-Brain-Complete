...
const OVERNIGHT_TASKS = [
  {
    ...
    tasks: [
      {
        id: "t1",
        text: "Review investor deck feedback from Sarah",
        priority: "high",
      ...
      {
        id: "t2",
        text: "Prepare talking points for board call",
        priority: "high",
      ...
      {
        id: "t3",
        text: "Send NDA to potential partner",
        priority: "medium",
        estimatedTime: "5 min",
        status: "pending",
      },
  ...
  },
  {
    ...
    tasks: [
      {
        id: "t4",
        text: "Finalize term sheet revisions",
        priority: "high",
      ...
      {
        id: "t5",
        text: "Schedule due diligence call",
        priority: "medium",
  ...
  },
  {
    ...
    tasks: [
      {
        id: "t6",
        text: "Review UX feedback from testing",
        priority: "medium",
      ...
      {
        id: "t7",
        text: "Approve new feature specifications",
        priority: "low",
        estimatedTime: "15 min",
        status: "pending",
      },
  ...
  },
  {
    ...
    tasks: [
      {
        id: "t8",
        text: "Review portfolio company updates",
        priority: "medium",
  ...
  },
  {
    ...
    tasks: [
      {
        id: "t9",
        text: "Complete leadership course module",
        priority: "low",
        estimatedTime: "30 min",
        status: "pending",
      },
...
export default function EveningReview() {
  ...
  useEffect(() => {
    if (
      reviewState === "in_window" &&
      !chiefOfStaffPrompt &&
      !isReadyToStart &&
      !autoProcessingStarted
    ) {
      ...
      if (sessionId) {
        const decisions = activeTasks.flatMap(project =>
          project.tasks.map(task => ({
            ...
            decision: getTaskStatus(task.id) as
              | "accepted"
              | "deferred"
              | "rejected",
  ...
  const getTotalStats = () => {
    ...
    const rejected = allTasks.filter(
      t => getTaskStatus(t.id) === "rejected"
    ).length;
    const deferred = allTasks.filter(
      t => getTaskStatus(t.id) === "deferred"
    ).length;
    const pending = allTasks.filter(
      t => getTaskStatus(t.id) === "pending"
    ).length;
    ...
    try {
      ...
      if (sessionId) {
        ...
        if (result.signalItemsGenerated > 0) {
          toast.success(
            `${result.signalItemsGenerated} items prepared for your morning Signal`
          );
        }
      }
  ...
  const getReviewStateText = () => {
    switch (reviewState) {
      case "before_window":
        return "Review window opens at 7:00 PM";
      case "in_window":
        return "Review window open";
      case "cutoff_warning":
        return "Less than 15 minutes until cutoff!";
      case "auto_processing":
        return "Auto-processing by Chief of Staff";
      case "completed":
        ...
        <div className="bg-amber-500/20 border-b border-amber-500/30 px-6 py-2">
          <div className="max-w-7xl mx-auto flex items-center gap-2 text-amber-400">
            ...
            <span className="font-medium">
              Less than 15 minutes until 8 PM cutoff!
            </span>
            <span className="text-amber-300/70">
              Chief of Staff will auto-process remaining tasks.
            </span>
          </div>
        </div>
      )}
                            ...
                            <div className="flex items-start justify-between gap-3">
                              ...
                              <div className="flex gap-1 shrink-0">
                                ...
                                <Button
                                  size="sm"
                                  variant={
                                    status === "deferred" ? "default" : "ghost"
                                  }
                                  onClick={() =>
                                    setTaskStatus(task.id, "deferred")
                                  }
                                  className={
                                    status === "deferred"
