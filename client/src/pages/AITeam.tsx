...
export default function AITeam() {
  ...
  const handleStartProject = (expert: AIExpert) => {
    setLocation(
      `/ai-experts?mission=${encodeURIComponent(`Work with ${expert.name} on ${expert.specialty}`)}`
    );
  };

  const handleSendForTraining = (expert: AIExpert) => {
    alert(
      `${expert.name} has been sent for overnight training. They will be enhanced by morning.`
    );
  ...
  const handleFireExpert = (expert: AIExpert) => {
    if (
      confirm(
        `Are you sure you want to replace ${expert.name}? A new expert will be generated.`
      )
    ) {
      alert(
        `${expert.name} has been replaced. A new ${expert.specialty} expert is being generated.`
      );
      setSelectedExpert(null);
    }
  };
    ...
    <div className="min-h-screen bg-black text-white p-6">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            287 AI Experts (250 Core + 25 Left Field + 5 Celebrity + 7
            Corporate)
          </p>
        </div>
      ...
      <Tabs defaultValue="individuals" className="space-y-6">
        ...
        <TabsContent value="individuals" className="space-y-6">
          ...
          <div
            className={
              viewMode === "grid" ? "grid grid-cols-4 gap-4" : "space-y-3"
            }
          >
            {filteredExperts.slice(0, 50).map(expert => (
              ...
              >
                ...
                >
                  <div
                    className={`text-4xl ${viewMode === "list" ? "" : "mb-3"}`}
                  >
                    {expert.avatar}
                  </div>
                ...
                <div
                  className={`flex items-center justify-between ${viewMode === "grid" ? "mt-3" : ""}`}
                >
                  <Badge
                    variant="outline"
          ...
          {filteredExperts.length > 50 && (
            <p className="text-center text-foreground/70 text-sm">
              Showing 50 of {filteredExperts.length} experts. Use search to find
              specific experts.
            </p>
          )}
        </TabsContent>
            ...
            {corporatePartners.map(partner => (
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <Badge
                    variant="outline"
                    className={`${getScoreBg(partner.performanceScore)} ${getScoreColor(partner.performanceScore)} border`}
                  >
            ...
            <div className="bg-gradient-to-br from-green-500/10 to-green-500/5 border border-green-500/20 rounded-xl p-6">
              ...
              <div className="space-y-3">
                {topPerformers.map((expert, index) => (
                  ...
                  >
                    <span className="text-2xl font-bold text-green-400/50">
                      #{index + 1}
                    </span>
          ...
          <div className="bg-gradient-to-br from-fuchsia-500/10 to-cyan-500/10 border border-fuchsia-500/20 rounded-xl p-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-black/20 rounded-lg p-4">
                ...
                <p className="text-3xl font-bold text-white mt-1">
                  {Math.round(
                    allExperts.reduce((acc, e) => acc + e.performanceScore, 0) /
                      allExperts.length
                  )}
                  %
                </p>
              ...
              <div className="bg-black/20 rounded-lg p-4">
                ...
                <p className="text-3xl font-bold text-white mt-1">
                  {allExperts
                    .reduce((acc, e) => acc + e.projectsCompleted, 0)
                    .toLocaleString()}
                </p>
              </div>
              <div className="bg-black/20 rounded-lg p-4">
                ...
                <p className="text-3xl font-bold text-white mt-1">
                  {allExperts
                    .reduce((acc, e) => acc + e.insightsGenerated, 0)
                    .toLocaleString()}
                </p>
              </div>
            </div>
            <div className="mt-4 p-4 bg-black/20 rounded-lg">
              <p className="text-sm text-foreground/80">
                ...
                Your team is performing well overall.
                {needsReview.length > 0 &&
                  ` ${needsReview.length} experts need attention - consider sending them for overnight training.`}{" "}
                Top performers are excelling in Investment & Finance and
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-white/10 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              {/* Performance Score */}
              <div
                className={`rounded-xl p-4 mb-6 ${getScoreBg(selectedExpert.performanceScore)} border`}
              >
                <div className="flex items-center justify-between">
                  <span className="text-foreground/80">Performance Score</span>
                  <span
                    className={`text-3xl font-bold ${getScoreColor(selectedExpert.performanceScore)}`}
                  >
                    {selectedExpert.performanceScore}%
                  </span>
                </div>
                <Progress
                  value={selectedExpert.performanceScore}
                  className="mt-2"
                />
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedExpert.compositeOf.map(influence => (
                    <Badge
                      key={influence}
                      variant="secondary"
                      className="bg-fuchsia-500/10 text-fuchsia-300"
                    >
                      {influence}
                    </Badge>
                  ))}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 border border-white/10 rounded-2xl max-w-2xl w-full">
            <div className="p-6">
              ...
              <div
                className={`rounded-xl p-4 mb-6 ${getScoreBg(selectedCorporate.performanceScore)} border`}
              >
                <div className="flex items-center justify-between">
                  <span className="text-foreground/80">Performance Score</span>
                  <span
                    className={`text-3xl font-bold ${getScoreColor(selectedCorporate.performanceScore)}`}
                  >
                    {selectedCorporate.performanceScore}%
                  </span>
                </div>
                <Progress
                  value={selectedCorporate.performanceScore}
                  className="mt-2"
                />
              </div>
