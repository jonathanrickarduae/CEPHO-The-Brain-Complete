...
Return only a JSON array of IDs: ["id1", "id2", ...]`,
  ...
  askFollowUp: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const expert = EXPERT_PROFILES[input.expertId] ??
        Object.values(EXPERT_PROFILES).find((e) => e.name.toLowerCase().replace(/[^a-z]/g, '') === input.expertId.toLowerCase().replace(/[^a-z]/g, '')) ??
...
Provide a concise, expert response (2-3 sentences).`;
      ...
      return {
        answer: completion.choices[0]?.message?.content ?? "I'd be happy to elaborate on that point further.",
        expertName: expert.name,
        expertRole: expert.role,
      };
    }),
  ...
  generateReportMarkdown: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const sections = (input.sectionReviews ?? []).map((s: Record<string, unknown>) => {
        const sectionTitle = String(s.sectionTitle ?? s.sectionId ?? 'Section');
        const score = s.overallScore ? `**Score: ${s.overallScore}/10**` : '';
        const insights = Array.isArray(s.expertInsights)
          ? s.expertInsights.map((i: Record<string, unknown>) => `- **${i.expertName}**: ${i.insight}`).join('\n')
          : '';
        return `## ${sectionTitle}\n${score}\n\n${insights}`;
      }).join('\n\n');

      const experts = (input.expertTeam ?? []).map((e: Record<string, unknown>) => `- ${e.name} (${e.role})`).join('\n');
