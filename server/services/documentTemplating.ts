...
const TEMPLATE_STYLES: Record<
  DocumentTemplate,
  { accentColor: string; headerStyle: string }
> = {
  ...
  strategic_memo: {
    accentColor: "#8b5cf6",
    headerStyle: "Strategic Memorandum",
  },
  innovation_proposal: {
    accentColor: "#f59e0b",
    headerStyle: "Innovation Proposal",
  },
...
function renderSectionHtml(section: DocumentSection): string {
  ...
  if (section.type === "bullets") {
    const items = section.content
      ...
      .map(
        line =>
          `<li style="margin-bottom:4px;">${line.replace(/^[-•*]\s*/, "")}</li>`
      )
      .join("");
...
function renderSectionMarkdown(section: DocumentSection): string {
  ...
  if (section.type === "bullets") {
    ...
    body = section.data.map(row => `**${row.label}:** ${row.value}`).join("\n");
  } else {
    body = section.content;
  }
  return `${heading}${body}\n\n`;
...
export function formatDocument(input: DocumentInput): FormattedDocument {
  ...
  const confidentialityLabel =
    CONFIDENTIALITY_LABELS[input.confidentiality ?? "internal"];
...
_Generated by CEPHO.AI_`;
  ...
  const plainText = [
    ...
    ...input.sections.map(s =>
      [s.heading ? `${s.heading.toUpperCase()}` : "", s.content]
        .filter(Boolean)
        .join("\n")
    ),
    "",
