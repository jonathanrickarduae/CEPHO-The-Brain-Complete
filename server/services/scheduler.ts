    ...
    try {
      ...
      for (const user of allUsers) {
        ...
        const completedToday = await db
          .select()
          .from(tasks)
          .where(and(eq(tasks.userId, user.id), eq(tasks.status, "completed"), lt(tasks.updatedAt, new Date())))
          .limit(20);
        ...
        await db.insert(activityFeed).values({
          ...
          metadata: { automated: true, jobId: "evening-review", completedCount: completedToday.length },
        });
      }
  ...
  cron.schedule("0 9 * * *", async () => {
    ...
    try {
      ...
      const staleTasks = await db
        .select()
        .from(tasks)
        .where(and(eq(tasks.status, "in_progress"), lt(tasks.updatedAt, threeDaysAgo)))
        .limit(50);
      ...
      for (const task of staleTasks) {
        await db.insert(activityFeed).values({
          ...
          metadata: { automated: true, jobId: "stale-check", taskId: task.id },
        });
      }
      log.info(`[Cron] Task Stale-Check — flagged ${staleTasks.length} stale tasks`);
    } catch (err) {
    ...
    try {
      ...
      for (const idea of unscoredIdeas) {
        ...
        await db
          ...
          .set({ confidenceScore: score * 10, status: "under_review", updatedAt: new Date() })
    ...
    try {
      ...
      for (const user of allUsers) {
        ...
        await db.insert(activityFeed).values({
          ...
          metadata: { automated: true, jobId: "agent-snapshot", completedCount: completedTasks.length },
        });
      }
    ...
    try {
      ...
      for (const user of allUsers) {
        const recentMoods = await db
          .select()
        .from(moodHistory)
        .where(and(eq(moodHistory.userId, user.id), lt(moodHistory.createdAt, new Date())))
        .orderBy(desc(moodHistory.createdAt))
          .limit(7);

        if (recentMoods.length >= 3) {
          const avgScore = recentMoods.reduce((s, m) => s + (m.score ?? 5), 0) / recentMoods.length;
          const trend = avgScore >= 7 ? "positive" : avgScore >= 5 ? "neutral" : "concerning";
          ...
          await db.insert(activityFeed).values({
            ...
            metadata: { automated: true, jobId: "mood-trend", avgScore, trend },
          });
        }
      }
    ...
    try {
      ...
      for (const user of allUsers) {
        await db.insert(activityFeed).values({
          ...
          description: "Your weekly KPI digest is ready. Review your performance metrics for the past 7 days.",
          metadata: { automated: true, jobId: "weekly-kpi", week: new Date().toISOString().slice(0, 10) },
        });
      }
    ...
    try {
      ...
      for (const user of allUsers) {
        const recentNps = await db
          .select()
          .from(npsResponses)
          .where(and(eq(npsResponses.userId, user.id), lt(npsResponses.createdAt, new Date())))
        ...
        if (!lastResponse || lastResponse.createdAt < ninetyDaysAgo) {
          await db.insert(activityFeed).values({
            ...
            description: "We'd love your feedback! Please take 30 seconds to rate your CEPHO.AI experience.",
    ...
    try {
      ...
      for (const user of allUsers) {
        await db.insert(activityFeed).values({
          ...
          description: "Your Digital Twin has been updated based on this week's activity. Review your profile to see what's changed.",
...
function scheduleSubscriptionAlerts() {
  cron.schedule("30 9 * * *", async () => {
    ...
    try {
      ...
      log.info("[Cron] Subscription Renewal Alerts — completed (pending full subscription data)");
    } catch (err) {
