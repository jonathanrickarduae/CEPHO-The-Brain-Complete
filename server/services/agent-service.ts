...
export class AgentService {
  ...
  async getAllAgents(): Promise<AIAgent[]> {
    ...
    const result = await this.db!.execute(
      'SELECT * FROM ai_agents ORDER BY performance_rating DESC'
    );
  ...
  async getAgentsByCategory(category: string): Promise<AIAgent[]> {
    if (!this.db) await this.initialize();
    const result = await this.db!.execute(
      'SELECT * FROM ai_agents WHERE category = $1 ORDER BY performance_rating DESC',
      [category]
    );
    return result.rows as AIAgent[];
  }
  ...
  async getAgentCapabilities(agentId: string): Promise<AgentCapability[]> {
    if (!this.db) await this.initialize();
    const result = await this.db!.execute(
      'SELECT * FROM agent_capabilities WHERE agent_id = $1 ORDER BY added_date DESC',
      [agentId]
    );
  ...
  async getDailyReports(agentId: string, limit: number = 30): Promise<DailyReport[]> {
    if (!this.db) await this.initialize();
    const result = await this.db!.execute(
      'SELECT * FROM agent_daily_reports WHERE agent_id = $1 ORDER BY report_date DESC LIMIT $2',
      [agentId, limit]
    );
  ...
  async getLatestReport(agentId: string): Promise<DailyReport | null> {
    if (!this.db) await this.initialize();
    const result = await this.db!.execute(
      'SELECT * FROM agent_daily_reports WHERE agent_id = $1 ORDER BY report_date DESC LIMIT 1',
      [agentId]
    );
  ...
  async getPendingReports(): Promise<DailyReport[]> {
    ...
    const result = await this.db!.execute(
      `SELECT r.*, a.name as agent_name, a.category 
       ...
       JOIN ai_agents a ON r.agent_id = a.id 
  ...
  async getPendingApprovals(): Promise<ApprovalRequest[]> {
    ...
    const result = await this.db!.execute(
      `SELECT r.*, a.name as agent_name, a.category 
       ...
       JOIN ai_agents a ON r.agent_id = a.id 
       WHERE r.status = 'pending' 
       ORDER BY r.risk_level DESC, r.cost_estimate DESC`
    );
  ...
  async getAgentApprovals(agentId: string): Promise<ApprovalRequest[]> {
    if (!this.db) await this.initialize();
    const result = await this.db!.execute(
      'SELECT * FROM agent_approval_requests WHERE agent_id = $1 ORDER BY created_at DESC',
      [agentId]
    );
    return result.rows as ApprovalRequest[];
  }
