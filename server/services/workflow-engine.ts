...
import { workflows, workflowSteps, workflowValidations } from '../../drizzle/schema';
...
/**
 * Workflow Engine - Core state machine for managing multi-step workflows
 */
export class WorkflowEngine {
  ...
  static async getUserWorkflows(
    ...
    if (skillType) {
      query = query.where(and(
        ...
        eq(workflows.skillType, skillType)
      ));
    }
  ...
  static async validateStep(
    ...
    for (const validation of validations) {
      ...
      await db.insert(workflowValidations).values({
        id: uuidv4(),
        workflowId,
        stepId,
  ...
  static async generateDeliverable(
    ...
    return {
      url: `/api/workflows/${workflowId}/deliverables/${deliverableType}`,
  ...
  static async deleteWorkflow(workflowId: string): Promise<void> {
    ...
    // Delete validations
    await db
      .delete(workflowValidations)
      .where(eq(workflowValidations.workflowId, workflowId));
