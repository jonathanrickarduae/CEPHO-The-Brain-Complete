...
const ENCRYPTION_KEY =
  process.env.INTEGRATION_ENCRYPTION_KEY || "cepho-integration-key-2026-secure";
...
export class IntegrationManager {
  ...
  private async testClaude(
    ...
    if (response.ok || response.status === 400) {
      // 400 is ok, means API key is valid
      return { success: true };
    } else {
