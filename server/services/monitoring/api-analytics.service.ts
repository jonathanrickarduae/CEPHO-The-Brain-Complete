...
export class ApiAnalyticsService {
  ...
  getAnalytics(): ApiAnalytics {
    ...
    const errorProneEndpoints = endpoints
      .map(e => ({
        ...
        method: e.method,
        errorRate:
          ((e.clientErrorCount + e.serverErrorCount) / e.requestCount) * 100,
      }))
  ...
  getEndpointAnalytics(
    ...
    const allMethods = Array.from(metricsStore.values()).filter(
    ...
    const totalRequests = allMethods.reduce(
    ...
    return {
      ...
      clientErrorCount: allMethods.reduce(
        (sum, e) => sum + e.clientErrorCount,
        0
      ),
      serverErrorCount: allMethods.reduce(
        (sum, e) => sum + e.serverErrorCount,
        0
      ),
      avgResponseTime:
        allMethods.reduce(
      ...
      firstAccess: new Date(
        Math.min(...allMethods.map(e => e.firstAccess.getTime()))
      ),
      lastAccess: new Date(
        Math.max(...allMethods.map(e => e.lastAccess.getTime()))
      ),
    };
  }
