...
export const featureComparison = pgTable("feature_comparison", {
  ...
  description: text("description"),
  theBrainStatus: text("theBrainStatus"),
...
export const regulatoryLandscape = pgTable("regulatory_landscape", {
  ...
  moatPotential: text("moatPotential"),
  description: text("description"),
...
export const expertCoachingSessions = pgTable("expert_coaching_sessions", {
  id: integer("id").generatedAlwaysAsIdentity().primaryKey(),
  expertId: varchar("expertId", { length: 50 }).notNull(),
  coachType: text("coachType").notNull(),
...
export const blueprints = pgTable("blueprints", {
  ...
  name: varchar("name", { length: 300 }).notNull(),
  description: text("description"),
  blueprintType: text("blueprintType").notNull(),
...
export const smePanels = pgTable("sme_panels", {
  ...
  projectId: integer("projectId"), // Optional link to project
  panelTypeCode: text("panelTypeCode").notNull(),
  name: varchar("name", { length: 200 }).notNull(),
...
export const qualityGates = pgTable("quality_gates", {
  ...
  level4Decision: text("level4Decision"),
...
export const manusDelegationLog = pgTable("manus_delegation_log", {
  ...
  delegationType: text("delegationType").notNull(),
...
export const ideaAssessments = pgTable("idea_assessments", {
  ...
  assessorType: text("assessorType").notNull(),
...
export const trendRepository = pgTable("trend_repository", {
  ...
  trendStrength: text("trendStrength").notNull(),
...
export const digitalTwinGoals = pgTable("digital_twin_goals", {
  ...
  profileId: uuid("profile_id")
    .references(() => digitalTwinProfile.id)
    .notNull(),
...
export const qmsAuditTrail = pgTable("qms_audit_trail", {
  ...
  performedBy: uuid("performed_by"), // Note: uuid type, references users.id (integer) - type mismatch, FK not enforced
...
export const qmsComplianceChecks = pgTable("qms_compliance_checks", {
  ...
  checkedBy: uuid("checked_by"), // Note: uuid type, references users.id (integer) - type mismatch, FK not enforced
...
/**
 * Agent Insights — daily insights generated by agents during research runs.
 * Appendix H / Table 57 (Phase 3)
 */
export const agentInsights = pgTable("agent_insights", {
  ...
  usedInBriefingId: integer("usedInBriefingId"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
...
/**
 * Agent Improvements — self-improvement suggestions from agents, pending approval.
 * Appendix H / Table 57 (Phase 3)
 */
...
export type InsertAgentImprovement = typeof agentImprovements.$inferInsert;

/**
 * Briefings — persisted Victoria's Morning Briefings.
 * Appendix K / Data Dictionary 2.2
 */
...
/**
 * Email Messages — AI-triaged and summarised email messages.
 * GAP 4: Email Intelligence (Phase 3)
 */
export const emailMessages = pgTable("email_messages", {
  ...
  fromAddress: varchar("fromAddress", { length: 300 }),
  fromName: varchar("fromName", { length: 200 }),
  toAddresses: json("toAddresses"),
  subject: varchar("subject", { length: 500 }),
  bodyText: text("bodyText"),
  aiSummary: text("aiSummary"),
  aiPriority: varchar("aiPriority", { length: 20 }).default("normal"),
...
/**
 * Meeting Notes — AI-extracted notes and action items from meetings.
 * GAP 5: Meeting Intelligence (Phase 3)
 */
export const meetingNotes = pgTable("meeting_notes", {
  id: integer("id").generatedAlwaysAsIdentity().primaryKey(),
  userId: integer("userId").notNull(),
  externalMeetingId: varchar("externalMeetingId", { length: 200 }),
  title: varchar("title", { length: 500 }).notNull(),
  attendees: json("attendees"),
...
/**
 * Briefing Preferences — personalisation settings for Victoria's daily briefing.
 * GAP 17: Victoria's Briefing Personalisation Engine (Phase 3)
 */
export const briefingPreferences = pgTable("briefing_preferences", {
  ...
  maxInsightsPerSection: integer("maxInsightsPerSection").default(3),
  updatedAt: timestamp("updatedAt")
    .defaultNow()
    .$onUpdate(() => new Date())
    .notNull(),
...
export type InsertBriefingPreference = typeof briefingPreferences.$inferInsert;

/**
 * Briefing Feedback — Victoria rates briefing sections to improve future briefings.
 * GAP 17: Briefing Feedback Loop (Phase 3)
 */
export const briefingFeedback = pgTable("briefing_feedback", {
  id: integer("id").generatedAlwaysAsIdentity().primaryKey(),
  userId: integer("userId").notNull(),
  briefingId: integer("briefingId").notNull(),
  sectionId: varchar("sectionId", { length: 100 }).notNull(),
  rating: integer("rating").notNull(),
...
/**
 * Agent Daily Reports — persisted daily reports generated by all 51 agents.
 * Victoria reviews these and approves/denies capability requests.
 */
export const agentDailyReports = pgTable("agent_daily_reports", {
  ...
  newLearnings: json("newLearnings"),
  suggestions: json("suggestions"),
  capabilityRequest: json("capabilityRequest"),
  approvalStatus: varchar("approvalStatus", { length: 20 }).default("pending"),
  approvedAt: timestamp("approvedAt"),
  approvedBy: integer("approvedBy"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
export type AgentDailyReport = typeof agentDailyReports.$inferSelect;
export type InsertAgentDailyReport = typeof agentDailyReports.$inferInsert;

/**
 * Agent Performance Metrics — real DB-backed performance tracking per agent per day.
 */
export const agentPerformanceMetrics = pgTable("agent_performance_metrics", {
  ...
  agentName: varchar("agentName", { length: 200 }).notNull(),
  date: timestamp("date").defaultNow().notNull(),
  tasksExecuted: integer("tasksExecuted").default(0),
  tasksSucceeded: integer("tasksSucceeded").default(0),
  tasksFailed: integer("tasksFailed").default(0),
  avgResponseMs: integer("avgResponseMs").default(0),
  totalTokensUsed: integer("totalTokensUsed").default(0),
  userRating: real("userRating"),
  improvementCount: integer("improvementCount").default(0),
  learningScore: real("learningScore").default(0),
...
/**
 * SME Review Triggers — automated queue for engaging SMEs at key workflow stages.
 */
export const smeReviewTriggers = pgTable("sme_review_triggers", {
  ...
  expertType: varchar("expertType", { length: 50 }).notNull(),
  expertIds: json("expertIds"),
...
export type InsertSmeReviewTrigger = typeof smeReviewTriggers.$inferInsert;

/**
 * Victoria Actions — autonomous action log for everything Victoria does.
 */
export const victoriaActions = pgTable("victoria_actions", {
  id: integer("id").generatedAlwaysAsIdentity().primaryKey(),
  userId: integer("userId").notNull(),
  actionType: varchar("actionType", { length: 100 }).notNull(),
  actionTitle: varchar("actionTitle", { length: 500 }).notNull(),
  description: text("description"),
  relatedEntityType: varchar("relatedEntityType", { length: 50 }),
  relatedEntityId: integer("relatedEntityId"),
  autonomous: boolean("autonomous").default(true),
  metadata: json("metadata"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
...
// ============================================================
// Agent1 Personal AI System Tables
// ============================================================
