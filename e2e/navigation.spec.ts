...
test.describe('Navigation', () => {
  ...
  test('should navigate to Nexus Dashboard', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /nexus|command center/i })).toBeVisible();
  });

  test('should navigate to Chief of Staff Dashboard', async ({ page }) => {
    await page.goto('/operations');
    await expect(page).toHaveURL('/operations');
    await expect(page.getByRole('heading', { name: /chief of staff|operations/i })).toBeVisible();
  });

  test('should navigate to Innovation Hub', async ({ page }) => {
    await page.goto('/innovation');
    await expect(page).toHaveURL('/innovation');
    await expect(page.getByRole('heading', { name: /innovation hub/i })).toBeVisible();
  });
  ...
  test('should navigate to Project Genesis', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /project genesis/i })).toBeVisible();
  });
  ...
  test('should handle 404 for invalid routes', async ({ page }) => {
    ...
    const url = page.url();
    expect(url.includes('/404') || url.includes('/nexus') || url.includes('/dashboard')).toBeTruthy();
  });
});
